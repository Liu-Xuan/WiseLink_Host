...
import { AeoValidationWriteAuthorizationService } from './aeo-validation-write.service';
...
export class AeoArtifactActionService {
  constructor(
    ...
    private readonly validationWrites: AeoValidationWriteAuthorizationService,
    @Inject(AEO_SIMILAR_SEARCH_PORT)
    private readonly similar: AeoSimilarSearchPort,
  ) {}

  async execute(value: unknown): Promise<AeoArtifactActionResult> {
    const request = normalizeActionRequest(value);
    let validationWriteAuthorization: AeoArtifactActionResult['validationWriteAuthorization'];
    try {
      // This is deliberately before the role gate and WorkItem fresh-read:
      // neither an ordinary WorkItem nor an activation-only host can act.
      validationWriteAuthorization =
        await this.validationWrites.verify(request);
    } catch (error) {
      return blocked(request, [mapValidationWriteError(readCode(error))]);
    }
    return this.executeAuthorized(request, validationWriteAuthorization);
  }
   ...
   * called. Existing role, permission-snapshot, WorkItem fresh-read, immutable
   * readback and Registrar CAS checks remain mandatory below.
   *
   * The validation receipt remains mandatory for execute(), which is used by
   * the explicit validation candidate-loop. This method must not be called by
   * an unauthenticated transport.
   */
  async executeFromAuthenticatedHost(
    value: unknown,
  ): Promise<AeoArtifactActionResult> {
    return this.executeAuthorized(normalizeActionRequest(value), null);
  }

  private async executeAuthorized(
    request: AeoArtifactActionRequest,
    validationWriteAuthorization: AeoArtifactActionResult['validationWriteAuthorization'],
  ): Promise<AeoArtifactActionResult> {
...
function normalizeActionRequest(value: unknown): AeoArtifactActionRequest {
  if (!isRecord(value)) {
    projectionError(
...
function blocked(
  ...
  validationWriteAuthorization: AeoArtifactActionResult['validationWriteAuthorization'] = null,
...
function mapValidationWriteError(code: string) {
  ...
  const mapped = stable.has(code)
    ? code
    : 'AEO_VALIDATION_WRITE_AUTHORITY_UNAVAILABLE';
  return blocker(
    mapped as AeoArtifactActionResult['blockers'][number]['code'],
    null,
    `${code}: 独立 validation-write receipt 未通过，保持零 artifact action。`,
  );
}
