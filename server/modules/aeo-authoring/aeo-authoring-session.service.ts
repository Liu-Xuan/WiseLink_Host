...
export class AeoAuthoringSessionService {
  ...
  async open(value: unknown): Promise<AeoAuthoringSessionResult> {
    const request = normalizeAeoAuthoringSessionRequest(value);
    ...
    try {
      ...
      return blockAeoAuthoringSession(request, observedAt, [
        blocker(
    ...
    if (precondition) {
      return blockAeoAuthoringSession(
        request,
        observedAt,
        [precondition],
...
export function normalizeAeoAuthoringSessionRequest(
  value: unknown,
): AeoAuthoringSessionRequest {
  if (!isRecord(value)) {
    projectionError(
...
export function blockAeoAuthoringSession(
  request: AeoAuthoringSessionRequest,
  observedAt: string,
