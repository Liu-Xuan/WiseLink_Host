...
export class ReviewConversationRepository {
  ...
  async loadPendingOpenClawTurn(input: {
    ...
    const actorContext = this.db.$with('review_actor_context').as(
      this.db
        .select({
          ...
          actorId:
            sql<string>`set_config('app.user_id', ${input.actorId}, true)`.as(
              'pending_review_actor_id',
            ),
        })
        .from(workItem)
        .where(
    ...
    const pendingTurn = this.db
      ...
      .orderBy(asc(reviewTurn.createdAt), asc(reviewTurn.turnNo))
      .limit(1)
  ...
  async loadPreviousOpenClawTask(input: {
    ...
    const [previous] = await this.db
      ...
      .from(actorContext)
      .innerJoinLateral(previousTask, sql`true`);
  ...
  async appendTextTurn(input: {
    ...
    const insert = async (executor: DatabaseExecutor) => {
      await executor.insert(reviewTurn).values({
        reviewTurnId,
    ...
    try {
      if (input.requestId.startsWith('dialogue-')) {
        const requestRef = input.requestId.slice('dialogue-'.length);
        if (
...
function assertIdempotentReplay(
  ...
  if (
    ...
    turn.requestedModel?.modelRef !== requestedModel?.modelRef ||
...
function validateEngineerInput(value: unknown): void {
  ...
  if (record.requestedModel !== undefined)
    parseExecutionModel(record.requestedModel);
