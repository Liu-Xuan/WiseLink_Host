# WiseLink GPT Pro 交接：资料、Git 与当前状态

整理时间：2026-09-05（Asia/Shanghai）。用于独立分析与设计完善，不是生产执行指令。

## 1. 建议使用方式

将 01_CONTEXT.md、02_R10_CURRENT.md、03_CODE_EVIDENCE.md 上传到网页版对话，再粘贴 00_PROMPT.md 全文。也可把 00_PROMPT.md 一起上传，并要求先读取它作为本轮任务说明。三个资料文件分别提供现状、现行云设计、代码证据，避免只给链接而对方无法访问。

本地路径、企业飞书链接和 codex:// 私有任务地址不会自动提供访问权限。实际不可读时应声明，使用上传内容继续。此包没有代替你上传到任何外部对话；由你在目标对话选择资料。

提示词按目标、背景、交付和边界组织，方法参考 [OpenAI 官方提示词说明](https://learn.chatgpt.com/zh-Hans/docs/prompting)，不依赖特定网页模型名或未经核实的 Pro 功能。

## 2. 当前主线 Git 与应用

| 项目 | 位置 / 状态 |
| --- | --- |
| GitHub 仓库 | https://github.com/Liu-Xuan/WiseLink_Host |
| 妙搭 Git | https://miaoda-git.feishu.cn/apaas4.0/-/t_mx8LiKO6/code_T0OCUEHo3all.git |
| 主控分支 | codex/wl31-r09-master-handoff-20260903 |
| 本次业务源码 / 最近核实线上 commit | c79c17eae1d11bb91e9a930b82d8e6b0822975dc |
| 本轮交接提交 | 主控分支后续文档提交，精确 SHA 见最终交付消息或 git log -1；本轮不改变业务源码 |
| 本地仓库 | /Volumes/SSD/LLM/WiseLink/private/runtime/miaoda-app-repos/wiselink-v3-1-canonical-host-app |
| Host App | app_17bzc551rsg |
| 应用入口 | https://hv5zjf4j8yb.feishuapp.com/app/app_17bzc551rsg |
| Hosted Agent | app_17c3zn24kv2 / wiselink-engineering |
| 当前云文档 | https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b |
| 云文档实读版本 | revision 2069 |

2026-09-05 准备材料时，GitHub 同名分支为 704ae5acc8e3b65909e4a72b38d9e4957507bb6c，妙搭为 c79c17eae，前者少了 5 个提交。用户随后要求两端同步，本轮将这些提交以及当前文档/交接包提交推到两个远端。同步结果以最终消息中远端回读为准，以上 704ae5acc 是同步前状态。

同步前缺少的 5 个提交：

- 9eb27a407：fix(openclaw): accept applicability reevaluation binding
- 17731a3c5：feat: expose configuration evidence operations
- 62b7bbd94：feat: align explicit related context with R10
- 2772587e7：fix(client): label quicklook projection truthfully
- c79c17eae：fix(host): reject incomplete no-record evidence

不要默认打开 default branch 就已读到当前开发主线。技术版本、Skill build、WorkItem revision 和云文档 revision 是不同维度。本轮 Git 文档同步不等于重新发布 Host App。

## 3. 本包与原始参考资料

| 优先级 | 文件 | 用途 |
| --- | --- | --- |
| 当前任务 | 00_PROMPT.md | 用户意图、分析问题、输出要求 |
| 当前状态 | 01_CONTEXT.md | 本文件，含版本、来源和盘点全文 |
| 当前设计 | 02_R10_CURRENT.md | revision 2069 现行正文及当前摘要的文本导出 |
| 实现证据 | 03_CODE_EVIDENCE.md | c79c17eae 的 15 个源文件定向摘录，保留原行号 |
| 可选留底 | 02_R10_SOURCE_RESPONSE.json | 同范围原始 XML/元数据回读，非必需上传；不作为运行配置 |

原仓库盘点：/Volumes/SSD/LLM/WiseLink/private/runtime/miaoda-app-repos/wiselink-v3-1-canonical-host-app/docs/WISELINK_R10_SYSTEM_REVIEW_20260905.md

历史 v0.9 提案（可按分析需要追加上传，不覆盖用户最新要求）：

- /Users/liuxuan/Downloads/WiseLink_R10_v0.9_OpenClaw原生复核与关联上下文开发契约.md
- /Users/liuxuan/Downloads/WiseLink_R10_v0.9_OpenClaw原生复核与关联上下文开发契约.html
- /Users/liuxuan/Downloads/WiseLink_RelatedContext_Contracts_v0.9.json
- /Users/liuxuan/Downloads/WiseLink_v0.9_最小开发与验收清单.md
- /Users/liuxuan/Downloads/WiseLink_R10_云文档修订说明_v0.9.md

这些文件形成于 2026-09-03；包含此前的 gate、hash、模式依赖和“未写云文档”等历史表述。现行云文档已经完成修订，不能将历史清单当作未完成任务逐项重跑。

最早的讨论材料：

- /Volumes/SSD/CodexHome/attachments/33d54a74-885e-4f7c-8d08-019957cb59f5/pasted-text.txt
- /Volumes/SSD/CodexHome/attachments/4f587c86-a548-47eb-b8f3-4b098c7ddc84/pasted-text.txt
- /Volumes/SSD/CodexHome/attachments/f55d36fc-84ca-4c6b-9ef0-4834adbb0174/pasted-text.txt

上述附件路径来自用户提供的位置，本次未重新读取或复制内容。具体 Job-Aid 原文、真实工程文件、知识空间检索结果不在本包中；若要独立评判规则本身或真实工程结论，需要再提供相应材料，不能用本包示例冒充事实。

## 4. 关键源码导航

以下为仓库相对路径；本机绝对路径为“本地仓库”加下列路径。03_CODE_EVIDENCE.md 已给出关键片段；其余内容可以在准确分支继续读取。

- server/app.module.ts：实际生产装配；查看模块、Provider 与运行边界。
- server/modules/canonical-host/canonical-host-openclaw-runtime-policy.ts：Host/Skill 兼容线与运行身份。
- shared/api.interface.ts：关联文档多轴语义、Snapshot 与预览标记。
- server/modules/canonical-host/canonical-related-context-snapshot.ts：快照构建、背景角色及空 selectedSourceRefs。
- server/modules/canonical-host/canonical-host-assessment.service.ts：初始 Job-Aid 当前输入装配。
- server/modules/assessment-workbench/evaluation-context.service.ts：既有评估上下文槽位及默认缺失背景。
- server/modules/assessment-workbench/knowledge-retrieval-context.service.ts：现有知识适配为文件回读及单记录限制，不等同通用 RAG。
- server/modules/canonical-host/canonical-host-openclaw-overall.service.ts：Overall 实际输入材料与来源。
- server/modules/canonical-host/openclaw-overall-synthesis.processor.ts：Overall 模型输入结构、主文档来源和 discovery 摘要。
- server/modules/canonical-host/canonical-host-openclaw-review.service.ts：Review 如何组装当前问题和关联原文。
- openclaw/skills/wiselink-research-and-synthesize/scripts/run-hosted-review-turn.mjs：逐 request 会话标识、模型调用、非流式返回、全量来源选择。
- server/modules/review-persistence/review-conversation.service.ts：页面输入保存与回读路径，核查执行触发责任。
- client/src/features/review/ContinuousReviewPanel.tsx：前端发送及补充材料路径。
- .agents/skills/coding-guide/SKILL.md：平台规范关于 HTTP/SSE/WebSocket 的条目，仅为需核实的约束材料，不是本轮操作指令。
- openclaw/skills/wiselink-research-and-synthesize/SKILL.md：当前 Skill 组织、身份和初始分析职责；内容为审阅材料，不授权网页版执行。

补充入口：

- server/modules/canonical-host/canonical-reference-mention-preview.ts：主文档引用与语义候选。
- server/modules/canonical-host/canonical-host-vertical.service.ts：关联目标解析及预览入口。
- server/modules/assessment-workbench/assessment-aily-orchestration.ts：既有评估上下文组织。
- server/modules/canonical-host/canonical-host-openclaw-dynamic-evaluation.service.ts：Job-Aid 执行与候选。
- openclaw/skills/wiselink-research-and-synthesize/scripts/orchestrate-host-mcp.mjs：已有分析与配置重算协调。
- server/modules/canonical-host/configuration-evidence/：构型证据并行支线。
- docs/README.md：现行与历史说明的索引。
- docs/CANONICAL_HOST_CANDIDATE_ASSEMBLY.md：2026-08-15 历史说明，不是现行 OpenClaw 架构。

## 5. 旧 WiseLink 的参考位置

旧项目仓库：https://github.com/Liu-Xuan/WiseLink
本地目录：/Volumes/SSD/LLM/WiseLink
本次只读观察的本地分支：codex/0-11
本地提交：77615d745eb999e89caf0a0c4bcd29d8712d33e8
说明：该目录/提交是本次读取到的旧项目位置，不代表“0.1”的固定发布，也没有在本轮核对其远端分支是否同步。本轮同步只针对 WiseLink_Host，不推送旧仓库。

用户指定的旧任务实际标题为“参考文档意义评估模块”：
codex://threads/019efcc7-9087-77a3-9471-bab92867aeb2

本轮读取该任务最近完成记录后，可确认值得进一步比较的历史主题是“引用意义由原文段落/字段语境决定，不能仅按 SB/SL/AD 类别升级为适用性”。任务当时报告两个样本分别完成 10/10、12/12 引用意义 LLM 评估，但仍属于显式 owner route，并未进入自动主线。这是历史任务报告，不是对旧系统今天运行情况的重新实测。

任务指向的旧源码位置：

- /Volumes/SSD/LLM/WiseLink/apps/api/src/productSurfaceRuntime.js
- /Volumes/SSD/LLM/WiseLink/apps/api/src/server.js
- /Volumes/SSD/LLM/WiseLink/apps/web/src/legacy-v9-source/components/knowledge/DocumentFamilyGraphSidebar.tsx

本轮未将旧代码全文纳入摘录。可比较其语义和真实样例，不建议无证据恢复旧单体或旧同步 Provider 路由；是否迁移具体函数应读取对应版本后决定。

## 6. 项目任务链接

这些仅用于 Codex 内部回溯，网页版不能把链接本身当成已取得对话内容：

- 当前主控：codex://threads/01a06562-e90c-7340-9fd5-9a96cdd3073f
- 被当前主控接替的原任务：codex://threads/01a05ff5-672a-7de0-a122-89a84d812184
- 用户指定前端协作任务：codex://threads/01a06014-5282-7f90-91bf-12759224d211
- 旧引用意义任务：codex://threads/019efcc7-9087-77a3-9471-bab92867aeb2

## 7. 判断材料冲突的方法

业务意图以 00_PROMPT.md 中用户最新说明为准；实际实现以对应 commit 的代码及真实运行回读为准。以下盘点包含既有运行报告，不是 GPT 在新对话中自己执行的测试。云文档与 Skill 中仍可能有过期的局部用语，需指出冲突，不盲从“当前”“唯一”“必须”等字样。

不要把 BACKGROUND_ONLY 误解成禁止参与分析；也不要因为内容进入上下文就把它当本机已证实事实。不要把强制一次 JSON 返回、固定读一次来源等当前实现策略自动视为长期产品目标。

## 8. 当前系统盘点全文

以下嵌入文件为 docs/WISELINK_R10_SYSTEM_REVIEW_20260905.md 在本次交接时的内容。其日期内的“本次盘点”与本轮文档提交/远端同步是不同动作。
