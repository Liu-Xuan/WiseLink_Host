-- Independent document workspaces reuse V2 storage and original authorization.
BEGIN;
...
CREATE POLICY translation_workspace_document_boundary ON translation_workspace AS RESTRICTIVE FOR ALL TO PUBLIC
  USING (subject_kind <> 'DOCUMENT_VERSION' OR (
    engineering_matter_actor_has_tenant(tenant_id)
    AND engineering_matter_document_owned_by_actor(tenant_id, document_version_id)));
CREATE POLICY translation_block_document_boundary ON translation_block_revision AS RESTRICTIVE FOR ALL TO PUBLIC
