...
更新日期：2026-09-07。此次处理源于用户转交的 Codex 越界提醒；不是 Host 业务错误或飞书 key 故障。以下历史事实保留各自时点，不代表当前授权。

## 当前有效授权

**2026-09-06 用户已要求停止主动向公开 GitHub 提交或推送，只向飞书 Host 的 `origin` 推送。2026-09-07 新主控接管时再次要求避免越权。** 这取代下面记录的 9 月 5 日 GitHub 分支长期授权；没有新的明确指示，不执行 GitHub 分支推送或 main 镜像同步。

本机默认推送目标为 `origin`；实际推送仍明确 remote、单一源/目标引用并采用非强制方式。`github` 只保留历史和只读核对用途；旧 refspec 与凭据不构成授权。不主动删除已公开引用、改写历史或修改仓库可见性。

2026-09-07 在已配置的 `.githooks` 中增加 `pre-push`：仅当 remote 名称是 `origin` 且 Git 解析后的目的 URL 等于本项目的妙搭远端时放行。其余目的（包括直接 GitHub URL、SSH URL、重定向到 GitHub 的 origin）拒绝。本地回归测试不连接或写入远端。此保护覆盖本检出中正常启用 hook 的 Git 推送；不会删除既有公开内容，也不承诺消除平台对历史事件的提醒。
