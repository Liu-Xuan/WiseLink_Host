...
更新日期：2026-09-13。此次处理源于用户对双远端开发同步规则的最新明确授权；不是 Host 业务错误或飞书 key 故障。以下历史事实保留各自时点，不代表当前授权。

## 当前有效授权（2026-09-13）

**用户已明确允许与飞书 Host 的 `origin` 一起向公开 GitHub `github` 同步同一开发分支。** 这替代 2026-09-06 的 origin-only 规则及旧的一次性 GitHub 限制；仅适用于明确指定的项目 `codex/*` 开发分支。

`origin` 仍是妙搭开发来源；实际同步须分别明确 `origin` 与 `github`、精确同名源/目标 ref，并采用普通非强制快进。禁止自动 main、标签、其他分支、`--all`、`--mirror`、force、删除引用、改写历史、修改可见性或绕过认证和凭据/敏感数据检查。

已配置的 `.githooks/pre-push` 同时放行两个精确 remote/URL 配对，但只接受一条 `refs/heads/codex/*` 同名源/目标更新、非删除且快进；其余目的、分支、批量更新及非快进更新拒绝。定向回归测试覆盖 origin、github、URL、分支、删除、多行和非快进边界，不连接或写入远端。

## 2026-09-06 至 2026-09-12 的历史边界（已被最新要求取代）

2026-09-06 用户曾要求停止主动向公开 GitHub 推送、只向飞书 Host 的 `origin` 推送；2026-09-07 新主控接管时再次遵守该边界。本段保留当时的授权背景与历史操作，不代表当前限制。

## 2026-09-05 的历史授权（已被后续要求取代）
