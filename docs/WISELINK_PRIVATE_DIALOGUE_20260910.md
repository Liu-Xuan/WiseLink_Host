# Host 私人对话与显式评估更新

本次接入 `/dialogues` 和单会话地址。普通消息由已配置的 Aily 直接处理；文档复核页的普通聊天入口转入私人对话。原 Review 历史、明确的更新评估和正式采用操作继续保留。

- 会话默认仅本人可读，可无关注任务，也可关联最多八个获准任务。历史列表只提供本人会话标识和创建时间；每次正文读取重新检查对象授权。
- Host 保存原始用户输入、Aily 公开回答、生成时工作稿版本及实际上下文。Aily 获得工作稿摘要与有界近期历史，不宣称读取全文或无限会话记忆。
- 用户可选择自己输入或回答中的原话，关联到指定任务，标记问题、假设、纠正、约束或澄清。纠正可以明确替代一条同任务的有效贡献；撤回不抹除既往使用记录。
- “粘贴飞书片段”保存为用户提交摘录，不能证明原生作者或原消息真实性；默认仅保存，未验证原生自动回流、共享讨论和长期记忆撤回。
- 只有明确的评估更新请求才调用 OpenClaw。请求固定目标、业务版本、工作稿版本和所选贡献版本，保留原话与必要相邻问答。候选工作更新不等于正式采用。
- 同一请求重试恢复已保存结果；Aily 结果不明时保留记录并显示未知，不以新请求伪装恢复。公开 JSON 错误、流式断开和异步受理有区别。

数据库迁移 `0030_personal_dialogue.sql` 已按 dev → main 正常环境迁移执行，读回环境差异为空。新增四张表使用现有 actor transaction 与 RLS；旧 attempt 关联 Aily 查询继续兼容。

本地验证覆盖原文 UTF-16 选段、私人读回、上下文继承授权、并发 CAS、贡献纠正/撤回、评估请求幂等及前端权限失效清理；隔离 PostgreSQL 使用真实迁移和 RLS。技术发布与实际 Aily → 贡献 → OpenClaw → 新工作稿 → Aily 读回的业务验收分别记录，不能相互替代。

线上接入修订：首次 release `7683769644624432059` 的私人列表返回 `ENGINEERING_MATTER_RUNTIME_AUTHORIZATION_UNAVAILABLE`。原因是普通浏览器 SQL 身份无法直接进入仅供后台服务使用的 actor transaction。新增仅对话控制器可用的 SQL 作用域：先验证妙搭 Hosted 原生最终用户和 `wl_session`，要求用户、租户、应用一致，再在独立 SQL 异步上下文内使用该真实 actor。原请求身份、后台 service-role 校验和 RLS 均保留。真实 SDK + 隔离 PostgreSQL 验证了并发身份隔离、作用域退出恢复，以及过期/错用户/错租户/系统调用/本地伪造入口的拒绝。

随后读回确认下游重复 `SessionResolver.resolve` 在服务 SQL 角色下无法读取原本仅认证用户可读的会话表。修复为先验证一次，并仅在当前精确 HTTP 请求的异步作用域复用这一结果；普通请求解析不缓存，过期仍拒绝，新请求不继承。`ReviewConversationService` 的嵌套授权也沿用同一请求身份，对象权限继续新鲜读取。会话表策略保持不变；上述真实 SDK/数据库检查已增加该角色差异回归，身份接口 25 项测试通过。

显式更新继续使用原 Review 表的浏览器写权限：会话创建与 Review 服务在原浏览器 SQL 身份中执行；仅冻结请求和版本检查进入受控服务作用域，同一事务中的 ReviewTurn 插入再恢复原浏览器身份。真实 SDK/数据库验证了服务角色不能直接插入 ReviewTurn、浏览器角色不能读私人冻结请求，且身份切换后并发版本检查仍成立。未扩大任何既有表策略。

实际读回：release `7683778985674738628` 已 finished，提交 `a912d93e8`。2026-09-10 13:51 正常 Chrome 入口创建私人会话 `ea809786-a154-40c8-8250-6cc730bc3506`，关联当前 SB 任务 `WI-33f5b8fa-53e3-44ec-8b9f-05bbd1e03df0`；会话列表、独立地址和保存后正文读回均成功。Aily 返回 `USER_REAUTHORIZATION_REQUIRED`，发送按钮停用，尚无新消息或评估更新。官方“发起对话与管理会话”授权点击被自动审批复核拒绝，已向用户请求该项明确确认；未绕过，也不把本次私人存储验收记为完整业务闭环成功。
