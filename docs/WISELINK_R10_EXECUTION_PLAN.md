...
继续交付真实资料的正常受理、授权目录与快览、原文阅读、初始候选和连续 Review：工程师连续追问，并至少一次纠正方向或补材料，云端自动执行、承接共同上下文、展示实际取证及判断变化。优先使用已有、获准、实际可读的非事故资料；必要时从正常产品入口接入内容真正不同的新真实资料，不改名重传旧文件或修改内容规避去重。第一批并行完成实际文件定位登记、缺失的受理短事务、数据库授权目录/快览、同请求或任务范围内的字节与解析复用。旧对象故障独立交平台支持，不作为开发、联调和正常技术发布的前置条件；新流程成功不代表旧事故解决。保留备份和历史来源状态，不覆盖旧对象或业务引用，不正式采用、不自动确认 ReviewAction，不建设恢复平台、双写、多后端切换或分布式缓存。

产品 Goal 状态：本轮早期实读为旧目标 BLOCKED，当时没有编辑/恢复接口，应用 UI 自动操作也被产品安全边界拒绝。结束前再次实读返回 `goal=null`，原因未获确认；**当前未设置 Goal，未擅自重建，也未标记旧目标完成**。本节是用户要求的修订目标及验收依据，不冒充应用内 Goal 已修改；是否重新设置由用户确认。代码工作按本轮新目标继续，不由这个控制面限制阻断。保存、安装、mock、自动领取或测试数量都不能代替真实连续使用与候选回写证据，不增加缺乏具体失败场景的 gate、hash、冻结契约或第二业务状态机。
