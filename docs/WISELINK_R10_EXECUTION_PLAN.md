...
| 4. 页面连续往返 | 用户指定内置浏览器后，真实新 SB 已正常受理 | `737-46-1061_Original.pdf` 已从正常页面上传，新事项 `WI-9894f475-16c2-4e8b-9539-aa20ae118b70` revision 3；受控 PDF 实际加载 18 页。未复制 Chrome 凭证或修改扩展权限；正在将既有自动执行配置迁移到该事项。初始候选和至少两轮 Review 尚未验收。 |
| 5. 真实取证与可选资料隔离 | 本地实现及开发库变更完成，待技术发布/实流 | 关联目标逐份隔离并保留原因；现有 ActionAttempt 的可空活动字段记录 Host 上下文准备与 SourceRef 解析，回合可回读并定位来源，不冒充模型阅读或正式采用。见 [实现记录](WL31_R10_EVIDENCE_ACTIVITY_20260906.md)。 |
