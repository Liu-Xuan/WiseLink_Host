...
本轮读取产品 Goal 的实际状态为 **BLOCKED（旧目标）**，不是 ACTIVE。当前工具只能读取或标记完成/阻塞，不能改写目标或恢复；应用 UI 自动操作也被产品安全边界拒绝，因此此处是用户要求的修订目标及验收依据，不冒充产品 Goal 已修改，也不把旧目标标为完成以绕过限制。代码工作按本轮新目标继续，不由这个控制面限制阻断。保存、安装、mock、自动领取或测试数量都不能代替真实连续使用与候选回写证据，不增加缺乏具体失败场景的 gate、hash、冻结契约或第二业务状态机。
