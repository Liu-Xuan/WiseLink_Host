...
本地在研适配保持全文输入不变，模型仅输出单元序号与译文，由适配器恢复原来源绑定并执行既有完整性校验；新增安全输出形态观测，不记录原文、推理正文或凭证。两组普通测试 117 pass / 0 fail；未发布、安装或重新实跑，不宣称已完成连续输出。下一步还需正常失败处理入口及真实候选验证，不能靠删除 checkpoint 或改旧记录强制重放。
