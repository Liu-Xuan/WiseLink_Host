...
共享 Drive 来源状态：六个用户提供的根目录已登记，Host 已具备按租户和 `sourceKey` 的递归扫描、逐页 checkpoint、候选身份快照与恢复入口；完整扫描才更新候选快照，避免部分分页把旧来源误判为消失。候选比较按 `sourceKey + providerObjectId` 隔离，不同根目录的同 token 不会互相覆盖；供应方无版本号时使用 `modifiedTime` 判断变化，两个变化标识都缺失时保守标为需复核的变化，不宣称内容未变。权限拒绝会作为可恢复 blocker 持久呈现并保留扫描 frontier，未知网络错误不会被当作空目录。开发数据库已实际执行 0040/0041 并核验表、唯一索引和 RLS；当前 checkpoint/候选快照均为 0 行。2026-09-12 03:52（日志 `2026091203522874B2ECC434DC2C349E18`）以 bot 读取首个目录仍返回 Feishu `1061004 permission_denied`，因此后台监控和自动增量尚未接通，不能借用户会话代替应用授权。
