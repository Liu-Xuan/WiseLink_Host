...
共享 Drive 来源状态：六个用户提供的根目录已登记，Host 已具备按租户和 `sourceKey` 的递归扫描、逐页 checkpoint、候选身份快照与恢复入口；完整扫描才更新候选快照，避免部分分页把旧来源误判为消失。开发数据库已实际执行 0040/0041 并核验表、唯一索引和 RLS；当前 checkpoint/候选快照均为 0 行。用户身份可读，但应用 bot 对首个目录实测仍返回 Feishu `1061004 permission_denied`，因此后台监控和自动增量尚未接通，不能借用户会话代替应用授权。
