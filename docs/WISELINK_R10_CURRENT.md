...
2026-09-12 最新代码增量：EngineeringIssueSearchService 的搜索协议已返回命中类型、范围、命中理由、根来源和限制，事项页面同步展示这些边界；仍只返回已保存的 `WORK` 问题，正文按精确 revision 和现有 ACL 展开，不代表全量统计。投影搜索会先取有限候选再按授权展开，`hasMore` 只由第 51 个可读命中决定，不把无权行计入提示。检索投影失败可由调用方注入授权读取后逐项恢复，失败仍保留 pending；SOURCE/RECORD 投影与在线恢复触发尚未接通。共享 Drive 扫描服务已返回候选变化 `NEW/CHANGED/UNCHANGED`，仍未自动受理来源或派发分析。数据库投影/扫描 checkpoint 迁移和真实后台来源权限尚未完成。
