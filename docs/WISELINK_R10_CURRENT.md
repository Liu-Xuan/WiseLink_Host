...
来源：[R10 云文档](https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b)，revision 2203。 本轮修订已逐块回读；第 12 节的历史记录仍保留原始时点。
...
当前目标仍为用户 2026-09-06 12:08 要求的页面自助连续评估闭环，并包含 M3 与 DLI 两模型真实验收，尚未完成。用户授权 Docs/uploads 内真实 PDF、多样本及单事项执行范围迁移；不得据此扩展角色、成员或凭据权限。此前 737-46-1053 的 24 页原文已在页面加载，M3 翻译 ATT-b185188a-6c6f-4cec-9fc3-15f8f53e9e34 / AQ-ef986d9dd7b54785bd448c2571602b54 在 18:08:50 以 length / HTTP400 取消，无候选、无 revision 推进；旧失败与 checkpoint 保留。c25 的输出分窗已安装但未以新任务验证。Host 0c36eb97f 发布后，新真实 787 PDF 于 19:15 正常页面受理为 WI-1c1e3f05-0807-4cd1-b43c-df4aea5e4be3，PARSE_PDF SUCCEEDED、revision 3，页面显示正式出版物编号而非文件名别名；仅此解析成功不能算完成初始分析。后续还需核对并对齐该事项的既有 Host scope/唯一 cron、自然生成初始候选、至少两轮 Review（含纠正方向或补材料）、取证活动和判断变化。全局模型管理角色仍待用户明确，未授予权限、改写全局默认或启动 DLI 工程生成；未确认 ReviewAction 或正式采用结论。
...
模型选择与生效（2026-09-06）：用户先选择内置 MiniMax-M3，官方管理轮次 7682319105639271376 完成单次 openclaw models set miaoda/minimax-m3，15:25:13 Gateway 热加载。随后用户要求接入此前 DLI、页面全局切换内置或外部模型、仅影响后续新启动分析，并要求两模型真实跑通。DLI 由用户自行登记；只读轮次 7682344940369775802 确认 dli/gpt-5.6-sol、available=true、上下文声明 200000、未显式声明输出上限；该次核验默认 M3、fallback 为空，未读取或传递密钥。c24 接通的模型页、受控全局设置、ActionAttempt 启动绑定与同一 Hosted Gateway 的 x-openclaw-model 路由保留在当前 Host/c25；恢复不重选，失败不换 Provider。此前页面实读四个内置选项和 DLI、M3/设置版本 0，但模型管理角色尚待用户确认，页面只读。18:05 M3 实跑失败，无候选；19:15 新 787 事项仅完成解析，尚未发起翻译或 DLI 工程生成。本轮没有角色/成员授权、全局默认写入或凭据变更；两模型完整初始候选、连续 Review 及模型切换写入验收仍未完成。
