...
2026-09-12 文档路线修订、2026-09-13 开始执行，依据 [统一实施计划](WISELINK_R10_UNIFIED_IMPLEMENTATION_PLAN_20260912.md)：Host 官方插件＋PDF.js 为默认解析生产者，Translation V2 接插件并保留局部恢复，工程分析只用确切原文且不等待中文。MinerU 模型恢复退出本批关键路径；M 负责 F1—F6、身份/云端消费与集成，P 负责解析、翻译和局部 Reader，按 H0/H1/H2 汇合。此为当前实施目标，尚不代表新路径已发布或运行成功。
