...
来源：[R10 云文档](https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b)，revision 2211。 本轮修订已逐块回读；第 12 节的历史记录仍保留原始时点。
...
2026-09-06 23:00 当前实证：任务级模型功能 aaac68ced 已于 20:39:22 发布；存储上传前只读传输重试及受理失败展示修订 50a45b12e 已于 22:48:42 发布（release 7682433168808643560，finished，error_logs 为空），官方 Hosted 保持 c25。19:15 受理的真实 SB-787-31-0019-01.pdf 对应 WI-1c1e3f05-0807-4cd1-b43c-df4aea5e4be3，PARSE_PDF SUCCEEDED、revision 3；本轮内置浏览器已加载全部 13 页原文。唯一 cron 与 Host 单事项 scope 已对齐该事项并启用，但旧失败的一小时调度退避仍保留，23:00 尚无新初始 attempt，实际 nextRunAt 为 23:09:44.307，不把启用时的临时重算时间当运行证据。22:51:55 另从页面正常创建 DLI 事项 WI-214ded11-e5b5-4580-972d-0344da0f51c7，根选择 dli/gpt-5.6-sol、解析成功、revision 3，等待后续空闲迁移。此前 M3 length / HTTP400 及 22:31 新 M3 事项的存储失败全部保留、不重放；新受理成功不证明旧对象恢复。两模型初始候选、连续 Review 和判断变化仍未完成。
...
复用 Turn/ActionAttempt 保存执行意图，官方 Hosted 唯一 command cron 保持每 60 秒、timeout 1800 秒、唯一 Skill 安装路径；c25 已安装，最新 Host 50a45b12e 已于 22:48:42 发布。c25 首次保留完整全文，在同一原生会话预先指定每次最多 96 个完整单元、约 6000 原文字符的输出窗，后续连续输出；每 tick 有序继续已就绪阶段，独立资料有界并行读取，CAS 写回及真实依赖保持有序。cron 已在空闲时迁至正常解析的 787 事项 WI-1c1e3f05-0807-4cd1-b43c-df4aea5e4be3，Host scope 对齐并启用；23:00 只读核查确认旧错误退避仍将首次自然运行排在 23:09:44.307，尚未产生新模型 attempt。没有手工 driver、cron run 或清理历史。DLI 新事项已受理并绑定所选模型，待前一事项空闲后按同一授权路径迁移。真实候选、连续往返、两模型和生产有界容错仍待完成。
...
当前目标仍为真实样本的页面自助连续评估，包含 M3 与 DLI 两模型验收，尚未完成。用户授权的真实 PDF、多样本、单事项 scope/唯一 cron 迁移及每任务模型方案持续有效；不再等待全局模型管理角色，不新增角色/成员或凭据权限。任务选择、持久化、实际执行绑定与跨模型新回合边界已随 aaac68ced 发布，50a45b12e 修复上传前只读传输失败和已登记失败的页面误报，并于 22:48:42 发布。M3 待跑事项 WI-1c1e3f05-0807-4cd1-b43c-df4aea5e4be3 的 13 页原文可读；cron/Host scope 已对齐并启用，旧失败退避使实际 nextRunAt 为 23:09:44.307，23:00 尚无初始模型 attempt。DLI 事项 WI-214ded11-e5b5-4580-972d-0344da0f51c7 于 22:51:55 页面受理成功并保存 dli/gpt-5.6-sol。接下来观察自然初始分析，再各完成至少两轮 Review（含纠正或补材料）、真实取证和判断变化；生产有界恢复仍待实现验证。旧失败不重放，普通候选不确认 ReviewAction 或正式采用工程结论。
...
模型选择与生效（2026-09-06）：官方 Hosted 已登记内置 MiniMax-M3 与用户配置的 dli/gpt-5.6-sol，未读取或传递密钥；TaskEnvelope/ActionAttempt 绑定和同一 Gateway 的 x-openclaw-model 路由继续复用。当前每任务选择已发布：初始阶段固定一个模型，Review 默认继承、只在新回合显式换模型；其他事项、历史结果、运行与恢复不受影响。普通账户已从页面看到两项选择，22:31 M3 新事项正确保存根快照但解析存储失败，22:51 DLI 新事项正确保存根快照且解析成功。M3 待跑的已有正常解析事项由首次初始 claim 固定既有部署初始选项，不能把目录可选、解析成功或配置路由当作模型已生成。M3/DLI 完整初始候选、连续 Review 与跨模型回合实测仍未完成；原全局设置写入口不再作为当前使用前置。
