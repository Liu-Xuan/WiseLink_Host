...
## 2026-09-15 当前产品规则：有效工程依据与事项认识

工程文件按自身最新有效版本评估，本版修订说明用于组织换版分析，实际判断仍核对新版正文、条件及以前评估。事项基于相关有效文件形成认识；参考参与理解、比较和风险分析，不跨文件改写受控要求或正式决定。来源用途按真实身份和任务确定，不按FTD/SB等名称一刀切，也不由相似度或普通引用制造强制依赖。

正式版次、DocumentVersion、parseRun、semanticRevision与workRevision分别表达；解析rev6不等于厂家R06。各family有效性由已有受控登记确定，未核实或最新版不可读时准确说明覆盖，不拿系统最新处理冒充最新有效依据。历史绑定不改latest。

正文v3、连续会话、动态问题组和Host分批保存保留。工作更正分清文件规定、事项解释和我方决定，不预设FTD必须影响或绝不影响SB；核对输入、原始生成、保存字段和实际读模型，再处理真正错误。有效表述可保留，虚假依赖可有据撤回；实际变化与模型理由分开，不强造新工作版本。

当前实施顺序以[执行计划](WISELINK_R10_EXECUTION_PLAN.md)为准：内容与自然运行闭合→文件自身换版评估→准确综合/Wiki/跨事项参考→后台来源变化与真实长SB。既有保存成果不清零，运行事实以[集成记录](coordination/M_INTEGRATION.md)顶部为准。下列带日期记录保留原观察时点，不覆盖本节当前规则。

前端由妙搭独立重构，消费相同工作和准确来源，M只统一数据含义及发布组合。Luna组织CodeM和Git，P守住原文/章节质量，豆包不在关键路径。不扩额、不重建翻译或第二知识库，不以保存次数替代内容验收。
...
2026-09-12 文档路线修订、2026-09-13 开始执行，依据 [统一实施计划](WISELINK_R10_UNIFIED_IMPLEMENTATION_PLAN_20260912.md)：Host 官方插件＋PDF.js 为默认解析生产者，Translation V2 接插件并保留局部恢复，工程分析只用确切原文且不等待中文。MinerU 模型恢复退出本批关键路径；M 负责 F1—F6、身份/云端消费与集成，P 负责解析、翻译和局部 Reader，按 H0/H1/H2 汇合。

2026-09-13 技术交付实证（本地进度补充，未声称云文档同步）：Host release `7684739961284201424` 已 finished，提交 `928924c074abc0b93b0f053b00cbbc371846b4a5`，错误列表为空；0043–0052 已发布，dev→online 结构差异为 0。0052 修正平台 PUBLIC/ALTER POLICY TO 的角色转换行为，三条文档 restrictive policy 的实际待发布 SQL 已确认同时覆盖 authenticated 与 service_role。正常已登录页面读取 10 份文档目录，并通过独立文档页实际读取 787 FTD 原件；旧解析失败记录保持不变。工程 Skill c85 已从 `58653b973` 生成且包内测试通过，已上传 Host 私有存储并回读校验 371535 字节及 archive SHA-256，尚未安装。单文档 scope 已绑定获准 787 FTD 的真实 owner 并回读确认，Host 真实服务调用仍待验证；云端确定性消费及 H1/H2 尚未闭环；技术发布不是插件实际运行、工程初评或正式采用的完成证明。

2026-09-12 最新代码增量：EngineeringIssueSearchService 的搜索协议已返回命中类型、范围、命中理由、根来源和限制，事项页面同步展示这些边界；仍只返回已保存的 `WORK` 问题，正文按精确 revision 和现有 ACL 展开，不代表全量统计。投影搜索会先取有限候选再按授权展开，`hasMore` 只由第 51 个可读命中决定，不把无权行计入提示。检索投影失败可由调用方注入授权读取后逐项恢复，Host 已提供受登录/对象入口保护的受控 rebuild 端点；投影迁移与 Drizzle schema 已对齐 `search_vector` 生成列及 GIN/范围索引，并提供只接受已授权 Reader 内容的 SOURCE/RECORD 写入原语，但查询消费者仍只展开 WORK，不能宣称来源检索已接通。共享 Drive 扫描服务已返回候选变化 `NEW/CHANGED/UNCHANGED`，仍未自动受理来源或派发分析。2026-09-12 05:07 以应用 Bot 对六个根目录逐一只读核验，全部返回 Feishu `1061004 permission_denied`（日志 `202609120507309E486A139C3591913489` 等），真实后台来源权限仍未完成。
...
7. 初始评估与交互评估分别由专属任务持续推进，项目主控协调共享接口、唯一消费者运行窗口和发布。视觉与 Atlas 独立并行；源码可按当前授权分别向飞书 origin 与 GitHub 同步同一 codex/* 开发分支。
...
共享 Drive 来源状态：六个用户提供的根目录已登记，SB/AD 作为已知但尚未取得根目录的未接通来源族显式保留，未伪造 token 或空扫描结果。Host 已具备按租户和 `sourceKey` 的递归扫描、逐页 checkpoint、候选身份快照与恢复入口；权限 blocker 与 frontier 一起持久化，成功重试后清除；完整扫描才更新候选快照，避免部分分页把旧来源误判为消失。单页超过批次上限时保存页内 `entryOffset`，恢复从同一页继续，避免重复或漏读。供应方重复分页 token 会记录 `DRIVE_PAGE_TOKEN_REPEATED` blocker 并保留 frontier，不会把漏项扫描当完成。候选比较按 `sourceKey + providerObjectId` 隔离，不同根目录的同 token 不会互相覆盖；供应方无版本号时使用 `modifiedTime` 判断变化，两个变化标识都缺失时保守标为需复核的变化，不宣称内容未变。未知网络错误不会被当作空目录。开发数据库已实际执行 0040/0041 并核验表、唯一索引和 RLS；当前 checkpoint/候选快照均为 0 行。2026-09-12 03:52（日志 `2026091203522874B2ECC434DC2C349E18`）以 bot 读取首个目录仍返回 Feishu `1061004 permission_denied`，因此后台监控和自动增量尚未接通，不能借用户会话代替应用授权。
...
初始评估与交互评估由两个专属可见任务并行负责，项目主控协调共同上下文、共享 DTO、数据库和发布。前端材质/目录与 Atlas 演示独立实施；生产资料、演示内容和真实业务证据分清，不未经协调混入主循环。源码按明确同名 ref 分别向飞书 origin 与 GitHub 非强制推送；Host/Skill 可在兼容窗口分别技术发布。
