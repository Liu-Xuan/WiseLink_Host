...
同步日期：2026-09-10（Asia/Shanghai）。

来源：[R10 云文档](https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b)，revision 2231。 本轮修订已逐块回读；第 12 节的历史记录仍保留原始时点。
...
5. **当前实证与目标分开。**截至 2026-09-10，M3 与 DLI 的普通讨论已有成功回答，显式更新能够冻结选中讨论范围；DLI 已实际读取原文并输出结构化候选，但证据标识错误与纠正超时使更新失败。JobAid 工作修订仍为 1，两模型完整初始分析与各至少两轮成功 Review 尚未完成。Aily 配置、应用权限与用户重新授权分别核实，当前状态见第 12 节。
...
| Base / Aily / 全量图谱 | Base 与全量图谱保留既有资产并后置。Aily 已进入用户授权下的按需检索接线，配置、应用权限、用户 OAuth 与真实调用结果分别核验；不成为已有材料分析的前置，返回内容不自动成为 Host 已核查原文。 |
...
2026-09-10 当前实证：c68 已完成官方安装，40/40 文件匹配、246 项检查通过，唯一 cron 已恢复 M3 单事项自然调度。普通 CHAT 已有多轮成功回答，显式更新已验证冻结选中讨论范围；M3 类型生成失败及 DLI 证据标识错误、纠正超时仍使工作更新未完成，JobAid 工作修订保持 1。下一步完成正常页面显式更新，再完成两模型初始分析和各至少两轮成功 Review；按第 12 节分别核验 Aily 授权与接入。安装、测试和技术发布不替代工作增量保存、取证与判断变化。
...
复用 Turn/ActionAttempt 保存执行意图，官方 Hosted 保持唯一 command cron、每 60 秒、timeout 3600 秒及唯一 Skill 安装路径。当前 c68 已安装，Host 6e97e4a5294395d4130a0acebfa715fc9e5ef9b5 的 release 7683695167439735736 finished；唯一 cron 已恢复 M3 单事项自然调度。翻译 v2 使用语义块、持久工作、短会话逐批生成与 Host 组装，已有保存通过正常新 attempt 接续；JobAid 按工程问题形成工作版本，Review 仅显式更新选中讨论。沿用当前授权、来源和 CAS，旧失败不重写，未知生成不自动重放。完整业务循环仍待完成，详细运行证据见第 12 节。
...
最近技术发布：Host 6e97e4a5294395d4130a0acebfa715fc9e5ef9b5，release 7683695167439735736 finished，WL_AILY_AGENT_ID 线上配置已随发布生效。官方 Hosted Skill 为 r09.c68，来源提交 3dda4ced1f00cfb98056a9a12545210923c3ba5e；安装回合 7683680984924376015 完成，正式目录 40/40 文件摘要匹配、246 项检查通过，唯一 cron 恢复 M3 自然调度。c68 改善已登记证据标识错误的反馈，不自动修补候选或放宽校验；尚无 c68 新业务更新成功证据。当前仅向妙搭 origin 非强制推送，GitHub 旧同步记录不构成继续推送授权。技术发布、应用授权与业务验收分别记账。
...
Aily 接入当前状态（2026-09-10）：用户已同意接入指定智能体 agent_4km47c77ujwqphg，Host 的 WL_AILY_AGENT_ID 配置已发布。飞书应用 cli_aadde8b579f95bc9 的用户身份 aily:agent_chat:write 原已开通；本次仅新增的 aily:agent_chat:read 保留“待发布、可调试”。未保存的版本 1.0.5 创建页已取消，未提交正式审核，企业审核规则与应用身份权限不变。09:11，在 read 已处于该状态后，正常 OAuth 链重新生成包含 read/write 两项 scope 的授权链接仍返回 20027，用户授权未完成；不能将开发者免审核调试路径表述为已在 WiseLink 标准 OAuth 链跑通。

权限方案评估：当前 Host 使用 POST stream:false 创建对话，再通过 GET 轮询结果；GET 要求 aily:agent_chat:read，不能仅删除 OAuth 中的 read scope。官方创建对话 API 支持 stream:true，SSE 最长 5 分钟，POST 只需已开通的用户身份 aily:agent_chat:write，因此 write-only 流式调用可作为减少新增权限的待选方案，尚未选择、实现或实测。流式连接中断后，write-only 方案没有远端结果补查权限，设计必须明确 UNKNOWN、本地已收结果持久保存与不自动重发，不能把传输中断判为远端未执行。

优先最小权限和已免审权限，开发者本人联调与其他普通用户正式使用分别核验；官方 OAuth 指南要求发布权限配置后构造含相应 scope 的授权链接。既有官方 API 对话返回两处知识空间条目、摘录与链接，仅证明该次调用及检索回答，不代表 Host 接入闭环、SSE 或群聊检索已接通。
