...
来源：[R10 云文档](https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b)，revision 2206。 本轮修订已逐块回读；第 12 节的历史记录仍保留原始时点。
...
| r09.c10 / r09.c25 | r09.c10 是普通 R09 Task 的最低兼容 build；当前官方 Hosted 安装 build 为 r09.c25，保留 c21 同轮按需来源读取、c22 跨轮授权会话、c23 统一初始分析/Review 入口和 c24 受控任务模型路由及有序阶段推进，新增全文上下文不变的预规划连续输出窗。安装不代表真实候选已完成。Turn 13 实际执行 c16、Turn 14 实际执行 c17，历史结果保留实际版本，不重标为新 build。 |
...
2026-09-06 当前实证：Host 0c36eb97f 已发布，官方 Hosted c25 已安装。此前真实 737-46-1053_R04.pdf 的 WI-6584f4cb-0a28-478e-9746-fe66acaf1b56 保留 18:05 自然启动 M3、18:08:50 length / HTTP400 失败记录，无翻译候选或后续 JobAid/Overall，未重放。新增真实 SB-787-31-0019-01.pdf 先因存储读取、再因新版 Publication 页眉身份识别失败，均未创建事项；Host 修复依据首页坐标恢复出版物行，而非按文件名猜身份。19:15 从内置浏览器“选择已上传 PDF”复用原对象并正常受理为 WI-1c1e3f05-0807-4cd1-b43c-df4aea5e4be3，页面显示 B787-81205-SB310019-00；数据库 PARSE_PDF SUCCEEDED、revision 3、79640 bytes、无受理错误。该新事项仅完成解析，尚无翻译或综合候选；其 PDF 查看器加载尚未取得本轮成功证据。安装、解析和任务启动均不等于初始候选完成；连续 Review、两模型实跑与判断变化继续按第 12 节验收。
...
任务级模型选择（2026-09-06 用户改案确认）：不再以修改全局默认作为当前使用入口。工程师创建工程事项时选择一次模型，该事项初始分析的全文翻译、适用性、Job-Aid 与综合评估沿用同一选择；后续 Review 默认继承，可在发起新一轮前显式选择其他已登记且获准使用的模型。选择在 Host 持久保存，已运行任务、恢复和同请求重试保留原绑定，历史结果保留实际模型来源，失败不自动切换 Provider。权限沿用现有事项/讨论操作权，不新增角色或成员授权；模型服务和凭据仍由官方 Hosted 配置管理，浏览器只提交模型标识，不提交 endpoint 或密钥。复用同一 Hosted OpenClaw、工程 profile、Turn/ActionAttempt 与 MCP；跨模型新回合承接 Host 保存的讨论和获准资料，不错误沿用另一模型的原生会话。M3 与 DLI 两模型都须完成真实初始分析及连续 Review，普通测试或安装不替代验收。此为当前实施方案，页面接线、持久化及真实验证正在进行。
...
复用 Turn/ActionAttempt 保存执行意图，官方 Hosted 原生 command cron 沿用每 60 秒配置和唯一 Skill 安装路径；c25 已安装，最新 Host 为 0c36eb97f。此前唯一 cron 与 Host scope 对齐 737-46-1053 后自然领取翻译，首轮以 length 失败，无新候选。c25 保持首次完整全文输入，在同一原生会话中预先确定每次最多 96 个完整单元、约 6000 原文字符的输出窗，后续连续输出，不拆成隔离的翻译任务。每个自然 tick 有序继续已就绪阶段，独立关联资料有界并行读取，保留 CAS 串行写回与真实输入依赖。19:15 新受理的 787 事项本轮未迁移 scope/cron、未手工运行 driver 或 cron run，仍待获准的单事项配置对齐和自然执行验证。按需取证、跨轮授权会话、共同上下文和 JobAid/Overall/Review 视图继续复用。真实候选、连续往返、两模型和生产有界容错尚未完成；开发中保留失败记录，不自动重放。
...
最近功能发布：Host release 7682377591382428892 于 2026-09-06 19:13:18 finished，commit 0c36eb97fbc6380352729e054481d58ed7f5ca79，error_logs 为空。c25 源提交 59550e642 和此次 Host 修复均以单一同名 refspec 非强制同步 origin 与 github 的 codex/wl31-r09-master-handoff-20260903，实际分别 4d38faea7 → 59550e642 → 0c36eb97f；未直接推 main、标签、删除引用或强推。Host 新增 Boeing Publication 页眉身份解析、稳定 422 错误及存储错误分类，客户端日志仅保留状态/稳定码/trace；无角色或数据库结构变更。有关普通测试与实际 PDF 本地测试共 87 pass，另 11 个未配置的无关真实文件用例 skipped；类型检查、生产构建通过。官方 Hosted c25 安装轮次 7682368307975376059 completed，26/26 源文件匹配，Ready/Visible，installed 三文件 127 pass / 0 fail；安装只更新唯一 Skill，无配置、cron 或业务调用。此前模型页、启动模型绑定、活动回执、可选来源隔离和存储读取优化继续保留。普通测试、安装、发布、解析受理和真实候选分别记账。
...
当前目标仍为真实样本的页面自助连续评估，并包含 M3 与 DLI 两模型验收，尚未完成。用户已授权 Docs/uploads 内真实 PDF、多样本及单事项执行范围迁移，并最新确认改为任务级模型选择：创建事项时选一次、初始阶段继承，Review 新回合可换，运行和恢复不换。不再等待全局模型管理角色确认，不新增角色/成员或凭据权限。此前 737-46-1053 的 M3 翻译在 18:08:50 以 length / HTTP400 取消，旧失败和 checkpoint 保留；c25 输出分窗已安装，尚待新任务验证。Host 0c36eb97f 发布后，新真实 787 PDF 于 19:15 页面受理为 WI-1c1e3f05-0807-4cd1-b43c-df4aea5e4be3，PARSE_PDF SUCCEEDED、revision 3，但初始候选尚未形成。当前先接通任务选择到实际执行链并兼容发布，再按既有授权对齐单事项 scope/唯一 cron，以自然 tick 完成初始候选及至少两轮 Review（含纠正或补材料）、真实取证活动和判断变化；生产有界恢复仍待实现验证。不手工运行 driver、不重放旧失败、不确认 ReviewAction 或正式采用工程结论。

容量诊断区分实测与推断：此前 auto 声明 contextWindow=200000、maxTokens=8192；M3 目录声明 maxTokens=16000，但真实上游 token 用量、最终输出及推理预算未知。737-46-1061 原文 27107 字符，503 单元传输 JSON 157319 字节，其中机械标识约 94.6 KB，不据此断言全文输入过大。c24 的 18:05 M3 运行只发出一次 driver HTTP 请求，Gateway 内部空结果重试 1/1 后仍 length / HTTP400，无可接续前缀。c25 因此在生成前规划最多 96 个完整单元、约 6000 原文字符的输出窗：首次保留完整全文，后续同原生会话续写，不切原文、不拼补截断 JSON、不提交半成品或切 Provider。该预算不冒充实际 token 上限；三个 installed 测试文件 127 pass、安装 Ready/Visible，不算真实生成。新的 787 事项已解析，真实连续输出、跨章节术语一致性及后续初始阶段仍待验证；带出处的摘要、元数据和术语锚点仍是后续增量。

模型选择与生效（2026-09-06）：15:25 用户选择内置 MiniMax-M3，官方轮次 7682319105639271376 完成单次 models set，Gateway 热加载。DLI 由用户自行登记，只读轮次 7682344940369775802 确认 dli/gpt-5.6-sol available=true；未读取或传递密钥。c24 接通的 ActionAttempt 启动绑定与同一 Hosted Gateway 的 x-openclaw-model 路由继续复用。用户现已明确同意将原全局默认切换方案改为每任务选择：工程事项初始阶段固定一个模型，Review 默认继承并允许新一轮显式换模型，其他事项、历史任务和恢复不受影响。任务选择只接受已登记且获准的标识，模型服务/密钥仍在官方配置中管理；不再为这一路径申请规则集工程 Owner 的额外权限，原全局设置写入入口不作为当前流程前置。任务级页面、持久化与跨模型会话边界正在接线，尚未发布验收。18:05 M3 旧任务失败、19:15 新 787 仅解析成功的事实保持；M3/DLI 完整初始候选、连续 Review 与切换实测仍未完成。
