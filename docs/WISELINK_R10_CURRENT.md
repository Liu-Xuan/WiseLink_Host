...
来源：[R10 云文档](https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b)，revision 2182。 本轮修订已逐块回读；第 12 节的历史记录仍保留原始时点。
...
| r09.c10 / r09.c23 | r09.c10 是普通 R09 Task 的最低兼容 build；当前官方 Hosted 安装 build 为 r09.c23，继承 c21 同轮按需来源读取与 c22 跨轮授权会话，增加统一初始分析/Review 消费入口。本地在研修改不自动成为已安装版本。Turn 13 实际执行 c16、Turn 14 实际执行 c17，历史结果保留实际版本，不重标为新 build。 |
...
2026-09-06 当前实证：官方 Hosted 唯一原生 command cron 已按用户授权迁移到新事项 WI-9894f475-16c2-4e8b-9539-aa20ae118b70，使用已安装 c23 的统一初始分析/Review 入口，每 60 秒自然消费。737-46-1061_Original.pdf 已从用户指定的内置浏览器正常受理，受控 PDF 阅读器实际加载 18 页；13:34 的自然 tick 领取翻译 AQ-6ae76040e7144305bf8a0cc812a267cb，13 个传输批次收齐 503 个原文单元，随后整篇模型调用以 stopReason=length / INITIAL_GATEWAY_HTTP_400 失败，未生成候选；失败 attempt 与 checkpoint 保留，后续 tick 只报告需处理，未重发模型。Host a7dad7e37 已发布上下文/来源实读回执及页面折叠活动区，并隔离可选关联来源失败；这些代码与发布证据尚不能代替新候选的页面实测。旧 Turn 15/16/17 保留原失败记录，初始候选、至少两轮连续 Review 与实际判断变化仍待验收。
...
复用 Turn/ActionAttempt 保存执行意图，官方 Hosted 原生 command cron 沿用每 60 秒配置和唯一 Skill 安装路径。c23 已安装，既有唯一 cron 已迁移到用户授权的新 SB 事项和统一入口；自然 tick 已实际领取初始翻译，但全文模型生成因长度终态失败，尚无新初始候选。优先修正全文输入/输出预算及机械字段开销，不默认切成独立片段，不擅自切换模型或伪造容量；用户随后明确选择内置 MiniMax-M3，已于 15:25 切换并热加载，详见第 12 节。c21 同轮按需取证与 c22 跨轮授权会话继续保留；Host a7dad7e37 已发布真实上下文/来源读取回执与可选来源失败隔离。共同上下文和 JobAid/Overall/Review 任务视图继续复用；翻译后续增量接入带出处的元数据、摘要和领域术语锚点，见第 10.4 节。消费者不能依赖开发者电脑、Codex 手动启动 driver 或另建业务状态机；当前开发不重放失败记录，实际候选与页面连续往返仍需验收。
...
最近功能发布：Host release 7682288523675814887 于 2026-09-06 13:29:09 finished，commit a7dad7e3732d5e4c8197bc8fd61d3a61d886d444，error_logs 为空；该提交已用单一同名 refspec 非强制同步 origin 与 github 的 codex/wl31-r09-master-handoff-20260903，未触碰 main、标签、删除引用或强推。Host 新增关联资料逐份失败隔离、ActionAttempt 可空真实活动记录及页面来源定位；发布后的 dev→online 数据库差异为空。它们证明 Host 已装配或解析来源，不冒充模型阅读或候选完成。官方 Hosted 同名 Skill 仍为已原位安装的 c23，安装轮次 7682263795376803027 completed、26/26 源文件一致并 Ready/Visible。此前 1abbfefce 的 locator 登记、受理短事务、数据库授权目录/快览与读取复用继续保留；构建与普通测试不代替页面连续使用证据。
...
当前执行目标仍为用户 2026-09-06 12:08 要求的页面自助连续评估闭环，尚未完成。用户授权使用 Docs/uploads 内不同真实 PDF、迁移单事项执行范围和扩大样本，并明确改用内置浏览器；737-46-1061_Original.pdf 已从正常页面受理为 WI-9894f475-16c2-4e8b-9539-aa20ae118b70，revision 3，受控 PDF 实际加载 18 页。Host exact WorkItem scope 与既有唯一 cron 已切至同一新事项，统一入口 consume-hosted-work-item.mjs；13:34:19.986 的自然 tick 实际领取 TRANSLATE，读完 13 个传输批次、503 个单元后调用全文模型，因 stopReason=length / incomplete_result / HTTP 400 失败，attempt AQ-6ae76040e7144305bf8a0cc812a267cb 已在提交前取消。无新候选、无事项版本推进，未手工运行 driver 或重放失败记录；初始候选和至少两轮 Review 仍待完成。

容量诊断区分实测与推断：故障发生时配置模型为 miaoda/miaoda-model-auto，声明 contextWindow=200000、maxTokens=8192；该次真实上游请求的 token 用量和最终生效预算未被记录，不能把声明值冒充实测限制。原文 27107 字符，503 单元传输 JSON 157319 字节，其中 unitKey/sourceRefId 等机械标识约 94.6 KB；现有证据更支持先检查输出预算与冗余字段，而非断言全文输入过大。保留全文的精简译文输出及安全输出形态观测已在本地通过 117 项普通测试，尚未发布、安装或重新实跑；保留全文连续输出已获用户认可，领域术语锚点/摘要/元数据只确立后续方向。页面仍将初始任务显示为暂无进度，是另一个待修正的真实接线缺口；不能将管理端错误记录当成页面已展示。

模型选择与生效（2026-09-06）：15:25 用户当时选择现成内置 MiniMax-M3，官方 Hosted 管理轮次 7682319105639271376 completed，只执行一次 openclaw models set miaoda/minimax-m3；默认文本模型由 auto 改为 M3，工程 profile 及其他无覆盖的文本任务继承该默认值，Gateway 于 15:25:13 热加载完成，无需重启。此历史操作没有接入外部服务或重放业务。用户随后明确要求接入此前 WiseLink 0.10 的 DLI、自助全局切换内置或外部模型，仅影响之后新启动的分析，两种模型都要通过真实页面流程。用户已自行在妙搭官方模型设置中添加 DLI GPT 5.6 Sol；只读管理轮次 7682344940369775802 completed，确认规范路由 dli/gpt-5.6-sol 已登记且 available=true，文本上下文声明 200000，未显式声明输出上限，原生默认仍为 M3、fallback 为空。没有读取或传递用户填写的密钥，也未据模型列表宣称已生成成功。本地 c24 已接通系统模型页、受控全局设置、ActionAttempt 启动选择与同一 Hosted Gateway 的 x-openclaw-model 路由；恢复不重选，未登记或调用失败不切换 Provider。翻译保留全文上下文并优先紧凑一次输出，必要时同会话连续输出；调度在同一次自然 tick 中有序继续已就绪阶段，独立资料读取有限并行。相关 7 组本地测试 52 项、Skill 本地测试及生产构建通过，仅证明本地实现；c24 安装、Host 发布、管理角色确认和 M3/DLI 真实初始候选及连续 Review 仍待完成，不以登记、安装、测试数量或手工 driver 代替验收。
