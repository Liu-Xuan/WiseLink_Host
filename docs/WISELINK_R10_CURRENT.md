...
来源：[R10 云文档](https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b)，revision 2160。
...
持久存储与恢复：Host 应保留实际 bucket、filePath、对象 ID 与既有版本关联；普通解析产物只保存逻辑 ref 并依赖当前默认桶的缺口需补齐，locator 不能由模型或浏览器任意指定。明确原件和必要产物的保留、文件级恢复与清理审计能力，采用受权限控制的恢复副本；数据库引用备份不能替代字节备份。开发/测试清理仅限明确归属的临时产物，不能按年龄删除受控原件。用正常产品上传、发布或重启后读取及一次恢复验证，不将短时探针、签名 URL 或 Agent 记忆视为长期持久性证明。这些是待实施的可靠性工作，不是本轮已解决声明，也不新增 hash、第二业务真值或 gate。
...
当前开发状态（2026-09-06 用户修订）：继续真实新资料受理、数据库授权目录/快览、原文阅读、自动初始候选及至少两个连续 Review，其中一轮纠正方向或补材料并解释判断变化。第一批实际 locator 登记、短事务、轻量目录/快览和同请求/任务读取复用已随 Host `1abbfefce` / release `7682262637312936935` 发布，c23 同名 Skill 官方原位安装及 117 项 installed 测试通过。11:49 真实页面目录返回当前账户 10 项资料，事故样本历史快览独立成功，两条 Trace 无 FileService span且明确原文未核验；新 PDF 受理被 Chrome 扩展本地文件访问权限阻塞，尚未上传/新建事项/切换 scope，初始候选与两轮 Review 仍未验收。产品 Goal 早期为旧目标 BLOCKED，结束前最新实读为 `goal=null`；未擅自重建或标记旧目标完成，是否重新设置需用户确认。详细修订目标与进度以 [当前执行计划](WISELINK_R10_EXECUTION_PLAN.md) 为准。旧对象故障独立交平台支持，不再是开发、联调和正常技术发布前置条件；新样本成功不能证明旧事故已解决。保留备份，不覆盖旧对象或历史引用，不重放失败回合，不正式采用，不新增恢复平台、双写、自动切后端、分布式缓存或无依据 gate。
