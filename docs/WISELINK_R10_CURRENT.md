...
同步日期：2026-09-06（Asia/Shanghai）。

来源：[R10 云文档](https://hv5zjf4j8yb.feishu.cn/docx/MA3fdjEycoISjHxptAqcsyxvn9b)，revision 2195。 本轮修订已逐块回读；第 12 节的历史记录仍保留原始时点。
...
| r09.c10 / r09.c24 | r09.c10 是普通 R09 Task 的最低兼容 build；当前官方 Hosted 安装 build 为 r09.c24，保留 c21 同轮按需来源读取、c22 跨轮授权会话和 c23 统一初始分析/Review 入口，增加受控任务模型路由、同全文上下文连续翻译输出及一次自然 tick 有序推进已就绪阶段。安装不代表真实候选已完成。Turn 13 实际执行 c16、Turn 14 实际执行 c17，历史结果保留实际版本，不重标为新 build。 |
...
2026-09-06 当前实证：Host 4d38faea7 与官方 Hosted c24 已发布/安装。用户授权的新真实 737-46-1053_R04.pdf 已从内置浏览器受理为 WI-6584f4cb-0a28-478e-9746-fe66acaf1b56，24 页、671 内容单元、672 来源引用；Host exact WorkItem scope 与原有唯一 command cron 已对齐这一事项。18:05:24 自然启动绑定 MiniMax-M3 的全文翻译，18:08:50 因网关 400/incomplete_result、stopReason=length 失败，未生成候选或执行后续 JobAid/Overall；页面实际从“进行中”转为“未完成”并说明无新候选。未选择飞机的适用性明确等待输入。此前 737-46-1061 的长度失败及旧 Turn 15/16/17 均保留，未重放。安装、任务启动和失败可见不等于候选完成；初始候选、至少两轮连续 Review、M3/DLI 两模型实跑与判断变化按第 12 节继续验收。
...
全局模型选择（2026-09-06 用户确认）：在 WiseLink 系统中选择全局默认模型，仅影响之后新启动的分析任务；已启动任务保留启动时模型，候选保留实际模型来源。复用同一官方 Hosted OpenClaw 与工程 profile，支持内置文本模型及用户此前 WiseLink 0.10 的 DLI 自定义模型，不另建直接 Provider 业务主入口。凭据只通过官方模型配置保存，Host 与浏览器业务接口仅传受控模型标识；模型失败不得静默跨 Provider fallback。内置 MiniMax-M3 和 DLI 均须跑通获准真实 SB 的页面初始分析及连续 Review，安装、模型列表、连接成功、mock 或手工 driver 均不代替验收。
...
复用 Turn/ActionAttempt 保存执行意图，官方 Hosted 原生 command cron 沿用每 60 秒配置和唯一 Skill 安装路径。c24 已安装，Host 4d38faea7 已发布，既有唯一 cron 经空闲禁用、修改 argv、Host scope 对齐和正常发布后再启用，已自然领取新的 737-46-1053 全文翻译，但首轮因长度终态失败，无新候选；没有手工启动 driver 或 cron run。保留全文输入，优先去除输出中的重复机械标识，必要时在同一全文上下文和原生会话中连续输出，不拆成隔离的翻译任务。每个自然 tick 有序继续已就绪阶段，独立关联资料读取有界并行，CAS 写回和真实输入依赖仍保留。c21/c22 的按需取证与跨轮授权会话、共同上下文和 JobAid/Overall/Review 任务视图继续复用。页面已实读初始阶段状态及启动模型；真实候选、连续往返、两模型验收和生产有界容错仍待完成。开发中保留失败记录，不自动重放；带出处的元数据、摘要和术语锚点仍按第 10.4 节作为后续增量。
...
# 12. 当前运行摘要与历史记录（2026-09-06）

最近功能发布：Host release 7682359509466106846 于 2026-09-06 18:02:38 finished，commit 4d38faea79b0db15382aef9a2d5f96e9339bb14f，error_logs 为空；该提交已用单一同名 refspec 非强制同步 origin 与 github 的 codex/wl31-r09-master-handoff-20260903，实际快进 a7dad7e37 → 4d38faea7，未直接推 main、标签、删除引用或强推。发布包含系统模型设置、ActionAttempt 启动模型绑定、初始进度回读、全文连续输出适配及有界资料读取；发布后 dev→online 数据库差异为空。官方 Hosted c24 安装轮次 7682357276456176840 completed，26/26 源文件与 Publish Lite manifest 一致，Ready/Visible，installed 三文件测试 125 pass / 0 fail。先前 a7dad7e37 的活动回执及可选来源失败隔离、1abbfefce 的 locator、受理短事务与数据库授权读取优化继续保留。普通测试、安装、发布和页面候选实跑分别记账。
...
当前执行目标仍为用户 2026-09-06 12:08 要求的页面自助连续评估闭环，并增加内置 M3 与 DLI 两模型的真实验收，尚未完成。用户授权使用 Docs/uploads 内不同真实 PDF、迁移单事项执行范围和扩大样本，并明确改用内置浏览器；新增 737-46-1053_R04.pdf 已从正常页面受理为 WI-6584f4cb-0a28-478e-9746-fe66acaf1b56，revision 3，受控 PDF 实际加载 24 页，解析记录 671 内容单元和 672 来源引用。原有唯一 cron 8db789fa-4fe9-4e58-850c-97b4cd70eb46 及 Host scope 已对齐新事项；18:05:24 自然创建翻译 ATT-b185188a-6c6f-4cec-9fc3-15f8f53e9e34 / AQ-ef986d9dd7b54785bd448c2571602b54，绑定 miaoda/minimax-m3、设置版本 0。18:08:50 首轮模型请求返回 400/incomplete_result，原生日志 stopReason=length，候选提交前取消；自然 tick 总耗时 210381ms，无候选、无 revision 推进。页面已实际显示“全文翻译·未完成”及失败说明；原失败和 checkpoint 保留，不重放。未手工运行 driver、确认 ReviewAction 或正式采用结论；此前 737-46-1061 的 AQ-6ae76040e7144305bf8a0cc812a267cb 也保留原失败记录。

容量诊断区分实测与推断：此前 737-46-1061 故障时配置 auto，声明 contextWindow=200000、maxTokens=8192；真实上游 token 用量与最终预算未记录。原文 27107 字符，503 单元传输 JSON 157319 字节，其中机械标识约 94.6 KB，不据此断言全文输入过大。c24 已安装保留全文的紧凑译文和同会话续写，但 18:05 的 M3 新样本在首轮仍以 length / HTTP400 失败，网关未返回可解析前缀，续写未执行。代码核对发现首轮仍要求优先输出全部单元，只靠模型自判预算提前收尾；在研修订将每次输出范围预先按最多 96 单元、约 6000 原文字符确定，全文仍在同一会话中，不切断或裁减原文、不拼补截断 JSON。它们是保守工作预算，不冒充实际模型 token 容量；需新的受控任务实测后再评价效果。5 项针对性普通测试通过，不算真实生成。初始进度页已实际显示运行及失败状态；带出处的摘要、元数据、术语锚点仍是后续增量。

模型选择与生效（2026-09-06）：15:25 用户当时选择内置 MiniMax-M3，官方管理轮次 7682319105639271376 completed，仅执行一次 openclaw models set miaoda/minimax-m3，默认文本模型从 auto 改为 M3，Gateway 于 15:25:13 热加载，无需重启。用户随后要求接入此前 WiseLink 的 DLI、自助全局切换内置或外部模型，仅影响后续新启动分析，两种模型都须通过真实页面流程。用户已自行在妙搭官方设置添加 DLI GPT 5.6 Sol；只读轮次 7682344940369775802 确认 dli/gpt-5.6-sol 已登记、available=true、上下文声明 200000、未显式声明输出上限，原生默认仍 M3、fallback 为空，未读取或传递密钥。c24 与 Host 4d38faea7 已安装/发布，接通系统模型页、受控全局设置、ActionAttempt 启动选择与同一 Hosted Gateway 的 x-openclaw-model 路由；恢复不重选，失败不换 Provider。页面实读 4 个内置选项和 DLI、当前默认 MiniMax-M3、设置版本 0；模型管理角色尚待用户确认，当前只读。18:05 新真实 SB 自然创建绑定 M3 的翻译任务，18:08:50 首轮以 length / HTTP400 失败，无候选；任务选择与实际上游来源分开记录，不能以模型目录或已启动宣称生成成功。相关 Host 7 组普通测试 52 项、installed Skill 测试 125 项和生产构建通过，但 M3/DLI 完整初始候选、连续 Review 及模型切换写入验收仍待完成。
