...
B3 发布后，同账号、`ALL`、空查询、首批20条加第21条探测的五个新页面 trace 为 `638537b843e3b90a8b43c16affede12f`、`72a3d85b6b72bcb3f023ebb8b5e4c6be`、`a28c86ec8346891171edd68558985bb4`、`c5d4810e2352e53814c4a4753b9aa042`、`f3fcffb0dba0274f83247baadec6cc89`。每次 SQL span 为205，较 B2 相同形状样本的245少40；app_server 中位9.944秒、app_web 中位10.853秒，均未达到发布前预定的9.38秒、10.38秒门槛。五次 `read_group_wait` 合计分别为7.778、10.051、9.848、4.591、9.813秒；`overview_origin_await` 各有22次，累计2.429、3.143、2.985、1.754、3.353秒。阶段计时并行、嵌套，不能相加或直接当成端点可节省时长。

B4 候选仅在现有最多4个根工作的目录窗口中，合并概述来源的确切历史修订与保存回执查询。原单条及新增批量路径共用同一来源选择谓词，逐根保留创建者、租户、修订上界、概述连续性和保存回执检查；被拒绝、缺失、无概述或损坏的根释放批次槽位，递归历史引用继续原单条读取。B3 的五次样本各有22次概述查询，其中根目录通常21次，预期结构性减少约15次查询调用；这只是依据当前样本形状的估计，需以受控发布后的实际 span、`overview_origin_batch_query`、`overview_origin_await`、`read_group_wait` 与端点重复样本核验，不能预先宣称提速。
