...
按用户 2026-09-18 补充，向运行中的妙搭会话发送纠偏后，操作员须额外点击该指令的“立即发送”，再核对会话消息确认指令进入执行。CLI 返回成功只表示消息已提交，不证明纠偏已经生效。

先检查现有队列，只对当前有效的纠偏执行上述操作；不得重复发送同一指令，也不得把过期材料或旧批次指令立即执行。浏览器不可用时明确记录“已入队、未确认执行”，继续不依赖纠偏的工作，不以等待超时推断会话结束或重新启动。
