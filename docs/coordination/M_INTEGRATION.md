...
本次仅选择性应用5bd167b0ed2dfb16199524c597530dfe2fdbc166和fa4bc31bf98c16ae1373d321c0fc3b232ef249b5的PDF增量，排除分支进度CURRENT_TASK/REPORT，保留5产品4测试与证据JSON。所有产品/测试与fa4逐字一致。Luna固定组合5套40项、clienttypecheck及diffcheck通过。包含精确身份与会话绑定、有界位置/zoom保存、新来源定位优先、滚动接管及仅缩放保存。

此为发布准备，不含后续03d0成员合并查询或c747 Skill公平续接。PDF实际两页样本往返仍待此次部署后验证，Hosted尚未安装。


## 2026-09-23 读取并行与资料库精确返回组合

上一组合81abd9b061cb1acb420d02752bd9d81a8a319d47已由release7688473071305133285回读finished、updated_at1790113088000、error_logs=[]，双远端同名一致。Luna正常身份验证保存修订16、资料关系3、图谱对象11/关系3及精确往返通过；B完整网络证据显示冷读取仍约2.65–4.17秒，未达目标。

当前81abd基线合并B至dd65cd04baab891ce32fc57c645b9e4d02ee5ddc：84acf授权成功后并行读取租户投影与来源身份，allSettled后保持原验证优先级和前后版本核对；dd65为图谱→全局资料库→产品返回按钮保留精确工作、镜头及既有显示上下文。两批均已Luna独立接受。并发代价明确为每成员可同时2读，保存读取4成员一组最多8，尚无线上收益证明。

固定组合Luna独立核对生产/测试文件与候选一致、A既有改动保留；后端3套70项、前端3套66项、双端typecheck和暂存diffcheck通过，无额外tracked修改。

本批不含随后PDF手动滚动恢复开发。当前只是集成准备，发布结果另据准确回执。Hosted包仍未完成安装，真实模型运行及竞争验收未进行。


## 2026-09-23 评估增量与事项快照组合

已发布f24b9714ec87f586cb5302185480ddcb7fc49e86，release7688462050381221075 finished、updated_at1790110424000、error_logs=[]，双远端同名SHA一致。Luna正常身份独立验证同M/W修订16知识与图谱准确往返及来源重读通过。B完整网络样本显示图谱来源新鲜期3次返回无请求，过期按预期重读；知识冷读仍约3.7–4.7秒，秒级目标未达成。

当前f24基线合并B至41e9c82b008a804b076ffb88542754cc0eb080e8（完整事项快照3SQL→1SQL，保留租户/RLS、三键关系、有序聚合及前后版本校验），同时准确选择性应用A f329bd997784c050e6a1001860535eedb7500aab和8c2a20bd913b160e1f4732226535fa8c5d4cbd26增量（保留未受影响来源分析覆盖；知识能力快照反映现有授权绑定，不自动检索）。全部产品批次已有Luna独立接受，无产品合并冲突。B隔离PG8/8为实现侧实际证据，独立验收未复跑PG，不冒充生产RLS。

固定组合Luna独立验收：7套99项、server typecheck及暂存diff-check全部通过；A增量与既有分页/原件准备正确叠加，B repository与41e9一致。本段是下一批集成准备，尚未发布。Hosted Skill尚未完成本次安装，不能以Host发布或候选测试替代真实评估全链验收。
...
第三批分支 `codex/integration-ab3-20260923` 基于dc491合并B至 `a88ec0daa3daf4049db5ee6c034df3c625139270`，固定包含77ae/fadf/e10/a7/0d66/a88，排除后续626dcf6。范围为元数据查询、消费者明确发布后有界接续、知识会话复用、图谱准确返回及普通段落来源位置恢复。独立组合前端82项、consumer132项、任务16项及双端types/lint/build通过，集合交叠不相加。真实PG本轮缺URL跳过，未将其算通过。

发布/安装待后续准确回执；Hosted新Skill尚未安装，知识30秒复用不承诺远端撤权瞬时感知，位置恢复只涵盖实际点击。线上验收必须真实侧栏/返回按钮和普通段落点击，不用手工URL代替。
...
实际切换采用备份c112目录后目录替换c113，再写入/更新`.openclaw/source-origin.json`，不是平台官方install命令，因此不把该来源文件当作平台安装回执。安装目录419/419 tests通过，skill ready，真实MCP metadata只读核验通过，34项必需工具齐全，`document_reading`与`get_pending_review_turn`可用。六条cron已按before快照恢复，逐条`enabled=true`、`lastRunStatus=ok`、`consecutiveErrors=0`，before/after精确一致。最后核对出现一个由恢复后cron正常启动、绑定`WI-d09b7acc-2221-4e51-addd-e040ce3c68f4`的工作项消费者，未涉及旧DRR且未终止。此证据不证明新模型的实际语义输出已通过。

## 2026-09-17 c113打包与过期状态修订

c113源码提交链123b3e560 → 09e7413a6 → f65689158已由Git协调员按同名开发分支同步origin/github。准确源f65689158f9b2f8b316c59950f28dd43d478a994已打包58文件/446210字节，archive SHA256 1a5fa4a9d7f97db72dcb21d2725470a9f369fa0f593f86a1284e553e6352356e，manifest SHA256 a8476756d19e9361a9b05ee6f6ec6121b6a0b9b605d4a4b8d8d5ad819008af3c；419项本地测试通过。后续安装目录与真实manifest核验已按上一节闭合。Host当前技术发布为25194577220312ea2f0d528026f1a0c332806ed9。

后台只读核查旧DRR-479689ba-b5a8-4be7-87e5-d7ca4e97ed15期限已过、无持有消费者进程，但Host仍RUNNING且result/revision为空；旧save.started使消费者仅查询STATUS、不再CLAIM，而过期更新此前只在CLAIM调用，因此一直PENDING_SAVE_CONFIRMATION。M最小修订STATUS：在既有来源与最终权限校验后调用现有、按DB期限和状态保护的expire，再回读真实状态；不修改期限、不覆盖SAVED、不触发模型或保存、不更改旧检查点。6项运行时测试与server类型检查通过；Astra独立审查与6项复跑接受。提交25194577220312ea2f0d528026f1a0c332806ed9已双远端同名同步；Host release7686410787204336938已finished、精确该提交、error_logs=[]。发布后仅执行一次旧DRR `READING_STATUS`，实际返回`status=EXPIRED`、`resultPresent=false`、`readingRevision=null`、`errorCode=DOCUMENT_READING_DEADLINE_EXCEEDED`、`lease=null`；未触发BEGIN、模型、SAVE或修改checkpoint。
...
M本地修订DocumentOriginalStore：删除SDK download之外的重复元数据请求，仍验证download返回对象metadata及bytes/SHA；新增loadForReading，只读校验已发布自包含manifest并核对raw描述符范围，普通read/loadPublished切换到该路径。发布/恢复/重用/换版分析仍load全量审计，前后权限和PUBLISHED校验不变。Astra独立查SDK与代码接受；3套11项（含实际execute/service.read）及server tsc通过。新增测试证明读取仅消费manifest、错误parseRevision拒绝、raw字节篡改仍被全审计拒绝。该修订已随145d84c及后继25194577技术发布；一次线上样本的parsing→reading→translation链仍约4.94秒，提速和全页秒级目标未验收。

妙搭Reader从初版补丁连续经过Astra反例审查和v3/v4/v5修订，最终v5以双方可取得的真实`45c3e525df9e0ac42e695c98560c157b5c6bb744`为基线，10文件、62343字节，patch SHA256 `82e6ce7b6c04f0281c7e4e74ed3485a80c04bbf8b8138b5f0f2eb052f351db38`、gzip SHA256 `c14828127cda17819a3fdfdfcd03590b091ad58ca1413756607b9052cd738a1d`。机械协议补交turn7686439071313431781已由固定验证器核对manifest、三块payload、双哈希、文件清单和clean45c apply-check通过；同一补丁在当前25194577也可干净应用。

最终实现隐藏阅读投影中的pageFurniture并保持连续正文，移除常驻流程提示和逐段页按钮；点击、键盘和多页选择均按实际sourceUnit定位，表格unit/row/cell保留稳定DOM id，选中文本不误跳，PDF按需自动读取。固定run正文不等待无关状态；translation仅在需要模式加载；status或translation的401/403/404会使同identity/session/epoch下的迟到正文与译文全部失效并保持拒绝态。多页选择绑定准确original.binding，切换parseRun或页集合时旧选择同步不可见且不可点击。Astra最终在隔离当前基线复跑5套58项及额外13项选页交互通过；M应用后本地3套39项、client typecheck、eslint、stylelint及diff check通过。未作生产PDF真读或完整视觉验收。
