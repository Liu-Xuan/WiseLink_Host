...
查实际模型指导发现workJson内的openQuestions四字段对象结构未说明（原函数的work schema已由字符串传输替代），只在被拒后才提供类型诊断。c108补齐首次指导中的准确对象字段、问题对象允许字段、正文引用与原文定位ID的区别。仅补传输说明，不改候选、Host校验、模型、纠正次数和额度；待验证安装及正常恢复。
