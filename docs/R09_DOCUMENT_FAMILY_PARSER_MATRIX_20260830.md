...
The controller serially replayed this successor onto `9b6ed3d4…`, which already
contained the accepted first family slice, OCR diagnostics/failure seam, FTD
package identity correction, configuration work, and frozen.2 runtime assets.
The current combined evidence below therefore follows the OCR owner's
authoritative page-level result rather than the pre-diagnostic source branch.
...
| Boeing Maintenance Tip   | `SB/机身/BOEING/2026/202603/787 MT 51-001-R3.pdf` (`cf622421…`, 6 pages) | Page 2 is `VISUAL_TEXT_UNVERIFIED`                     | **PROFILE/CLASSIFICATION ACTIVATED / OCR REQUIRED** — `issuer.boeing.maintenance_tip.v1` / `boeing.maintenance_tip`             | `maintenance_tip`; current producer returns `PDF_OCR_REQUIRED_UNSUPPORTED` before package/correlation. No pre-diagnostic whole-file success is retained.              |
...
| Airbus AOT               | `…/AOT-A32N033-24 R03.pdf` (`1170448b…`, 43 pages)                       | Pages 4, 27–28, 37–38, 42 are `VISUAL_TEXT_UNVERIFIED` | **PROFILE/CLASSIFICATION ACTIVATED / OCR REQUIRED** — `issuer.airbus.operator_transmission.v1` / `airbus.operator_transmission` | `operator_transmission`; current producer returns `PDF_OCR_REQUIRED_UNSUPPORTED` before package/correlation. Directory placement does not override page completeness. |
...
| Airbus Concession                 | `…/B-32QQ Concession.pdf` (`cc43a7e2…`, page 5 visual-text-unverified)     | **OCR AND TAXONOMY BLOCKED**; current first failure is `PDF_OCR_REQUIRED_UNSUPPORTED` | Page 5 must be materialized before the unresolved support-document taxonomy can be evaluated. The broad adapter also covers AME-like inputs, so neither OCR nor taxonomy is bypassed.                                                   |
...
- The Boeing SL profile remains recognizable, but its actual page 9 is likewise
  ...
  evidence.
- Boeing MT page 2 and Airbus AOT pages 4, 27–28, 37–38, and 42 are also
  OCR-required in the combined current. They are activated family
  classifications, not package/Reader successes, until the single OCR provider
  resolves those pages.
