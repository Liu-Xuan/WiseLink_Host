...
| Boeing SB          | `SB/机身/BOEING/2026/202605/737-34-3830 Original.pdf` (`add32c7d…`, 22 pages) | pages 7 and 21 require OCR | **PROFILE_VERIFIED / OCR_REQUIRED** — `issuer.boeing.service_bulletin.v1` / `boeing.sb`; no package is emitted until the raster regions are materialized | The family profile no longer depends on this file's digest or exact document number. Page-level raster diagnostics now correctly prevent the former text-layer-only package from being treated as complete.                                  |
...
| Boeing SL          | `SL-777-45-006.pdf` (`5cc88899…`, 11 pages)                                   | page 9 requires OCR        | **PROFILE_VERIFIED / OCR_REQUIRED** — `issuer.boeing.service_letter.v1` / `boeing.sl`; no package is emitted until the raster region is materialized     | Family narrowing plus content identity prevents referenced FTD/SB numbers from stealing the profile. Page-level coverage is authoritative over the earlier all-pages-text-layer assumption.                                                  |
