...
- Reader 1A 已从工作树提交并同步；四个既有未跟踪调试文件继续隔离。Reader 1B（真实语义 anchor 的双语独立滚动同步、受控 PDF boxes 高亮）尚未实现，不能把 1A 描述为完整 Reader 视觉/来源验收。图谱遗留 hook 的 mock 与旧 raw endpoint 已由 `b050b6e70` 收敛，筛选弹窗键盘/焦点行为由 `ca97db47c` 修复；完整 Cytoscape 图谱视觉、真实关系模型和正式入口仍未完成。失败的依赖安装没有改动 `package.json` 或锁文件。
