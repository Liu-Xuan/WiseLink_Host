...
- 当前文档 HEAD 为 `codex/0-11` 的 `6c14ba337aaa63960516b58a824972b968b6fb23`，父提交 `66c41c554a973b4a87fcaacdf29501f6638104aa`；`origin/codex/0-11` 与 `github/codex/0-11` 已分别回读相同 SHA。`1031e3caa` 仍是最近已接受的生产 Reader/换版代码恢复点，它把正式入口迁回真实授权读取链，并把 Suite 构造页限制在 `dev-preview`。之后已选择性提交 Reader 1A、图谱读取清理、筛选可访问性、图谱确切原文路径、Reader 来源定位优先级和无投影状态提示修复。祖先链还包含针对 `9419d320c` 已知身份回归的纠正 `8d186d5b2`、知识读取修复 `957f327cd` 和资料库真实默认选择 `d7b074447`。
...
- Reader 1A 已从工作树提交并同步；`25c34281a` 修复 URL 来源与页面内译文来源选择的优先级，四个既有未跟踪调试文件继续隔离。Reader 1B（真实语义 anchor 的双语独立滚动同步、受控 PDF boxes 高亮）尚未实现，不能把 1A 描述为完整 Reader 视觉/来源验收。图谱遗留 hook 的 mock 与旧 raw endpoint 已由 `b050b6e70` 收敛，筛选弹窗键盘/焦点行为由 `ca97db47c` 修复，`2c6691ea5` 恢复事项→确切原文→问题分析阅读路径；完整 Cytoscape 图谱视觉、真实关系模型和正式入口仍未完成。失败的依赖安装没有改动 `package.json` 或锁文件。
- Reader 1B 的当前技术边界已核对：`DocumentOriginalResult.locations` 保存真实 `pageIndex`、坐标空间和 `boxes`，但 `DocumentSourceReadingWorkspace` 仍通过 `DocumentOriginalInlinePreview` 使用按 `documentVersionId` 授权的 PDF blob iframe；已有 `PdfDocumentViewer` 则依赖另一套 `workItemId + CanonicalPdfPreviewProjection.opaqueLocator` 链。两者不能直接拼接或用滚动百分比代替，因此 1B 必须先统一受控 PDF 读取契约，再实现真实 box overlay；当前证据不足以宣称已完成。
