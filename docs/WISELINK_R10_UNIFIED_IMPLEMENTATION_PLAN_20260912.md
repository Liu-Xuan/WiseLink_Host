...
2026-09-12 进一步完成代码接线：Host 按 `PRUN-*` 派生幂等 `MW-*` taskId，轮询 Worker 到终态，校验每个签名产物的 URL、长度和 SHA-256，将受控结果转换为现有 MineruParseResult，再复用 Host `MineruArtifactStore`、逐项读回和 parseRun stage/publish CAS。服务端类型检查及 38 项相关测试通过；独立 Worker/Hosted 真实 parseRun 仍待运行验收。
