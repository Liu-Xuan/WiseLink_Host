...
这批 Host 修订本地实际 PDF 身份测试及有关普通测试 87 pass；另外 11 个无关真实文件路径未配置的用例明确 skipped。前后端类型检查、生产构建通过（已有父目录 tsconfig / module / chunk-size 警告保留）。本地结果、技术发布、页面受理和模型结果分别记账。

## 19:13 发布及 19:15 页面受理复测

Host 源提交 `0c36eb97fbc6380352729e054481d58ed7f5ca79` 已在 origin 与 GitHub 的单一同名 Codex 分支非强制快进 `59550e642 → 0c36eb97f`，没有 main、标签、删除引用或强推。正常 release `7682377591382428892` 于 19:13:18 finished，完整 commit 匹配、error_logs 为空；此版无数据库结构变更。

原保留失败请求的内置浏览器页签已不在列表中，先按原 developmentRunToken 只读查询，仍为零事项。新开正常页面后，在“选择已上传 PDF”列表选中 18:34 的 `SB-787-31-0019-01.pdf`，复用同一已上传对象，没有重复上传。只提交一次，19:15:33.605 创建 `WI-1c1e3f05-0807-4cd1-b43c-df4aea5e4be3`，19:15:43.202 PARSE_PDF SUCCEEDED、revision 3、无错误；文档版本为 `document_version_b91b79a4b966a02d8bab1cc0`，实际源文件 79640 bytes。页面跳转到新事项并显示正式出版物编号 `B787-81205-SB310019-00`、尚无综合候选。

本轮数据库 ActionAttempt 回读只有 PARSE_PDF 成功，没有翻译或其他模型阶段。后续点击“查看原文与解析”时，浏览器返回该页签已不属于当前会话，因此未取得新 PDF 查看器加载成功证据，未反复新开页签。原件本地 13 页验证不冒充线上查看器证明。本轮没有迁移新事项的 Host scope/唯一 cron、启动 driver、DLI 工程生成、全局默认写入、角色/成员授权或凭据变更。后续需核对并对齐既有单事项执行配置，以自然 tick 验证 c25 初始分析及连续 Review。

R10 云正文与第 12 节当前摘要已精确更新八处并于 revision 2203 回读一致，正文镜像及执行计划同步；历史 737、c24 和旧 Turn 失败事实不改写，资源块保留。
