...
const databaseUrl = process.env.DOCUMENT_READING_TEST_DATABASE_URL;
...
const source = { parseRunId: 'PR-test', parseRevision: 1, semanticRevision: 1, manifestSha256: 'a'.repeat(64),
  expectedRevision: 0 };
const range = { offset: 0, unitIds: ['unit-test'], anchorIds: ['a1'], nextOffset: null };
const command = { candidate: { schemaVersion: 'wiselink.document.reading.v1', headline: 'Fixture reading' },
  producer: { skillVersion: 'isolated-test', modelVersion: 'not-called' } };
const clients = [];

// This suite exercises the real repository and migrations against isolated PG.
// Minimal materialized fixtures intentionally bypass model/candidate validation,
// which is covered by document-reading-candidate.spec.ts; no model is invoked.
async function actor(role, userId) {
  ...
  return { client, repository: new DocumentReadingRunRepository(drizzle(client)) };
}
function materialize(run, revision) {
  return { schemaVersion: 'wiselink.document.reading.v1', candidateOnly: true,
    readingRunRef: run.runRef, readingRevision: revision, headline: 'Fixture reading', brief: { text: 'Fixture', quotes: [] }, explanation: [], criticalConditions: [], limitations: [],
    sourceBinding: { original: { documentVersionId: scope.documentVersionId, parseRunId: source.parseRunId, parseRevision: 1 }, semanticRevision: run.semanticRevision },
    readCoverage: { status: 'PARTIAL_DELIVERY', deliveredUnitIds: ['unit-test'], totalUnitCount: 2, sourceCoverage: {} },
    sourceAnchors: [], producer: command.producer, savedAt: '2026-09-16T00:00:00Z' };
}

test('independent document reading PostgreSQL request, leases, CAS, immutable revisions and source RLS', { skip: !databaseUrl }, async t => {
  const url = new URL(databaseUrl);
  assert.ok(['127.0.0.1', 'localhost'].includes(url.hostname));
  assert.match(url.pathname, /^\/wiselink_reading_test(?:_[a-z0-9_]+)?$/u);
  ...
  try {
    ...
    const activityRepository = new DocumentActivityRunRepository(drizzle(one.client));
    const activityCommand = { candidate: { schemaVersion: 'wiselink.document.activity-candidate.v1', statements: [] }, producer: command.producer };
    const activityRun = await activityRepository.begin(scope, { ...source, requestId: 'existing-activity', selection: { sectionIds: ['section-test'] } });
    const activityFence = await activityRepository.claim(scope, activityRun.runRef, 'activity-producer');
    const activitySaved = await activityRepository.save(scope, activityFence, activityCommand, (run, revision) => ({
      schemaVersion: 'wiselink.document.activity-candidate.v1', candidateOnly: true, runRef: run.runRef,
      candidateRevision: revision, statements: [{ statementId: 'existing-statement', label: 'Existing source activity' }],
      ...
      producer: command.producer, savedAt: '2026-09-17T00:00:00Z',
    }));
    const activitySnapshot = await admin`SELECT * FROM dm_document_activity_run ORDER BY run_ref`;
    ...
    await t.test('only one lease wins; exact delivery and SAVE replay retain one immutable revision', async () => {
      ...
      assert.equal(saved.readingRevision, 1);
      ...
      await assert.rejects(one.client`UPDATE dm_document_reading_run SET result_json='{}'::jsonb WHERE run_ref=${row.runRef}`, /TERMINAL_IMMUTABLE/);
      await assert.rejects(admin`DELETE FROM dm_document_reading_run WHERE run_ref=${row.runRef}`, /TERMINAL_IMMUTABLE/);
      const privateFields = await browser.client`SELECT lease_token,lease_owner,lease_expires_at FROM dm_document_reading_run WHERE run_ref=${row.runRef}`;
    ...
    await t.test('authorized readers see exact saved revisions; another tenant and revoked readers do not', async () => {
      assert.deepEqual(await browser.repository.readSaved({ ...scope, actorUserId: 'actor-two' }, source.parseRunId, 1, 1), saved);
      assert.equal(await browser.repository.readSaved({ ...scope, actorUserId: 'actor-two' }, source.parseRunId, 1, 9), null);
      assert.equal(await foreign.repository.readSaved({ ...scope, actorUserId: 'actor-foreign' }, source.parseRunId, 1, 1), null);
      await admin`DELETE FROM work_item WHERE requested_by_user_id='actor-two'`;
      assert.equal(await browser.repository.readSaved({ ...scope, actorUserId: 'actor-two' }, source.parseRunId, 1, 1), null);
      assert.equal(await foreign.repository.readSaved({ ...scope, tenantId: 'tenant-foreign', actorUserId: 'actor-foreign' }, source.parseRunId, 1), null);
    ...
    await t.test('concurrent requests with the same expected revision cannot both SAVE', async () => {
      ...
      assert.equal((await one.repository.readSaved(scope, source.parseRunId, 1)).readingRevision, 2);
      assert.deepEqual(await one.repository.readSaved(scope, source.parseRunId, 1, 1), saved);
    });

    await t.test('semantic revisions have independent CAS and exact saved reads; activity is unchanged', async () => {
      const secondSource = { ...source, semanticRevision: 2 };
      const row = await one.repository.begin(scope, { ...secondSource, requestId: 'semantic-two' });
      const fence = await one.repository.claim(scope, row.runRef, 'semantic-two');
      await one.repository.recordDelivery(scope, fence, range);
      const result = await one.repository.save(scope, fence, command, materialize);
      assert.equal(result.readingRevision, 1);
      assert.equal(result.sourceBinding.semanticRevision, 2);
      assert.equal((await one.repository.readSaved(scope, source.parseRunId, 1)).readingRevision, 2);
      assert.deepEqual(await one.repository.readSaved(scope, source.parseRunId, 2, 1), result);
      assert.equal(await one.repository.readSaved(scope, source.parseRunId, 3), null);
      await assert.rejects(one.repository.begin(scope, { ...secondSource, requestId: 'semantic-two-stale' }), /REVISION_CONFLICT/);
      assert.deepEqual(await activityRepository.readSaved(scope, source.parseRunId), activitySaved);
      assert.deepEqual(await admin`SELECT * FROM dm_document_activity_run ORDER BY run_ref`, activitySnapshot);
    });
    ...
    await t.test('reclaimed, cancelled and expired leases cannot publish a later result', async () => {
      ...
      await admin`UPDATE dm_document_reading_run SET lease_expires_at=CURRENT_TIMESTAMP-interval '1 second' WHERE run_ref=${row.runRef}`;
      ...
      await assert.rejects(one.repository.save(scope, old, command, materialize), /LEASE_REJECTED/);
      ...
      await admin.unsafe('ALTER TABLE dm_document_reading_run DISABLE TRIGGER document_reading_run_guard');
      await admin`UPDATE dm_document_reading_run SET deadline_at=CURRENT_TIMESTAMP-interval '1 second' WHERE run_ref=${deadline.runRef}`;
      await admin.unsafe('ALTER TABLE dm_document_reading_run ENABLE TRIGGER document_reading_run_guard');
...
async function reset(admin) {
  ...
  await admin`INSERT INTO dm_document_semantic_revision
    (tenant_id,document_version_id,parse_run_id,parse_revision,semantic_revision,actor_user_id,profile_ref,original_manifest_sha256,map_json)
    VALUES ('tenant-test','DV-test','PR-test',1,2,'actor-one','test',${source.manifestSha256},
      ${admin.json({ schemaVersion: 'wiselink.document.semantic-map.v1', profileRef: 'test', semanticRevision: 2,
        binding: { documentVersionId: 'DV-test', parseRunId: 'PR-test', parseRevision: 1 } })})`;
