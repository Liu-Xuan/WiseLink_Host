...
import {
  graphReadingParams,
  readGraphReadingState,
  withGraphReturn,
  type SuiteGraphReadingState,
} from './suite-graph-return';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { useCurrentUserSession } from '@client/src/app/providers/CurrentUserSessionProvider';
import {
  matterDocumentRoute,
  matterReferencedWorkRoute,
  matterWorkRoute,
...
import {
  buildSuiteGraphTimeline,
  type SuiteGraphTimelineEvent,
} from './suite-graph-timeline';
import {
  buildSuiteDomainGraph,
  buildSuitePanoramaGraph,
} from './suite-graph-perspectives';

export default function SuiteMatterGraphPage({
  matterId,
}: {
  matterId: string;
}) {
  ...
  if (workRefs.length > 1 || (workRefs.length === 1 && !workRef.trim()))
    return <p role="alert">指定工作身份无效，请从准确工作重新进入。</p>;
  return (
    <MatterGraphContent
      key={JSON.stringify([matterId, workRef, sessionGeneration])}
      matterId={matterId}
      workRef={workRef}
      session={sessionGeneration}
      denied={authenticationRequired}
    />
  );
}

function MatterGraphContent({
  matterId,
  workRef,
  session,
  denied,
  ...
  const [perspective, setPerspective] = useState<SuiteMatterGraphPerspective>(
    navigation.state.perspective ?? 'matter',
  );
  const read = useSuiteMatterGraph(matterId, workRef, session, denied);
  const historical = Boolean(read.graph?.historical);
  const catalog = useMemo(
    () => read.workspace?.matter.catalog.entries ?? [],
    [read.workspace],
  );
  ...
  const documentsGraph = useMemo(
    () => (matterGraph ? suiteDocumentPerspective(matterGraph) : null),
    [matterGraph],
  );
  ...
  const loadDomain = useCallback(
    (cursor: string | undefined, signal: AbortSignal) =>
      getCanonicalLibraryDocuments({ limit: 50, cursor }, signal),
    [],
  );
  const loadPanorama = useCallback(
    (cursor: string | undefined, signal: AbortSignal) =>
      getEngineeringMatterDirectory({ limit: 20, cursor }, signal),
    [],
  );
  const domainKey = useCallback(
    (item: CanonicalLibraryDocumentSummary) => item.familyId,
    [],
  );
  const panoramaKey = useCallback(
    (item: EngineeringMatterDirectoryResponse['items'][number]) =>
      item.matterId,
    [],
  );
  const domainPage = useSuiteGraphDirectoryPage(
    perspective === 'domain' && !historical && !denied,
    session,
    loadDomain,
    domainKey,
  );
  const panoramaPage = useSuiteGraphDirectoryPage(
    perspective === 'panorama' && !historical && !denied,
    session,
    loadPanorama,
    panoramaKey,
  );
  ...
  const panoramaGraph = useMemo<SuiteMatterGraphRead | null>(() => {
    ...
    return buildSuitePanoramaGraph({
      directory: {
        items: panoramaPage.items,
        nextCursor: panoramaPage.nextCursor,
      },
  ...
  const openWiki = () =>
    open(
      read.graph?.workRef
        ? matterWorkRoute(matterId, read.graph.workRef)
        : `/matters/${encodeURIComponent(matterId)}`,
    );
  const openProcess = () =>
    open(
      `/matters/${encodeURIComponent(matterId)}/process${read.graph?.workRef ? `?${new URLSearchParams({ workRef: read.graph.workRef })}` : ''}`,
    );
  const locateEvidence = (evidence: DocumentAssessmentEvidence) =>
    open(
      matterDocumentRoute(
        matterId,
        evidence,
        'brief',
        read.graph?.workRef ?? undefined,
      ),
    );
  const openEventTimeline = useCallback(
    (event: SuiteGraphTimelineEvent) => {
      const pins = event.pins;
      if (!pins) return;
      const returnQuery = graphReadingParams(
        matterId,
        read.graph?.workRef ?? (workRef || null),
        displayState.current,
      );
  ...
  const handleStateChange = useCallback(
    (state: SuiteGraphReadingState, beforeNavigation = false) => {
      ...
      const structural = JSON.stringify([
        ...
        state.page ?? 0,
        state.density ?? 4,
        state.relationMode ?? 'aggregated',
        state.layoutMode ?? 'force',
        state.perspective ?? 'matter',
        state.eventId ?? null,
        state.wikiTab ?? 'knowledge',
      ]);
      ...
      persistTimer.current = setTimeout(() => {
        persistTimer.current = null;
        persistState(displayState.current);
      }, 320);
    },
    [persistState],
  );
  const openTarget = (target: SuiteMatterGraphTarget) => {
    if (target.kind === 'evidence') {
      if (target.evidence.kind === 'DOCUMENT_PASSAGE')
        locateEvidence(target.evidence);
      else if (
        target.evidence.kind === 'PRIOR_RESULT' &&
        target.evidence.sourceWork
      )
        open(matterReferencedWorkRoute(target.evidence.sourceWork));
      return;
    }
    if (target.kind === 'statement') {
      const event = timeline.events.find(
        (item) =>
          item.pins?.statementId === target.statement.statementId &&
          item.pins.documentVersionId === target.documentVersionId,
      );
      if (event) openEventTimeline(event);
      return;
    }
    if (target.kind === 'matter-node') {
      if (target.matterId !== matterId)
        navigate(
          `/graph?${new URLSearchParams({ matterId: target.matterId })}`,
        );
      return;
    }
    const version =
    ...
    const query = new URLSearchParams();
    if (target.kind === 'input' && target.binding.original)
      query.set('parseRunId', target.binding.original.parseRunId);
    open(
      `/document-versions/${encodeURIComponent(version)}${query.size ? `?${query}` : ''}`,
    );
  };
  if (read.error)
    return (
      <section role="alert">
        <p>{read.error}</p>
        <button onClick={() => void read.refresh().catch(() => undefined)}>
          重新读取
        </button>
      </section>
    );
  if (read.loading || !read.graph)
    return <p role="status">正在读取事项及指定保存工作…</p>;
  ...
  const directoryPage =
    perspective === 'domain'
      ...
      : perspective === 'panorama'
        ...
        : null;
  const unavailable =
    historical && (perspective === 'domain' || perspective === 'panorama');
  const perspectiveNotice = unavailable
    ? '指定历史工作没有绑定该视角数据。请切回事项或工程文档；历史工作身份保持不变。'
    : directoryPage?.error
      ? `读取该视角失败：${directoryPage.error}`
      : !displayRead
        ? '正在读取该视角的授权数据…'
        : null;
  const combinedNotice =
    [read.warning, perspectiveNotice].filter(Boolean).join(' ') || null;
  // Keep the view mounted so tabs, return state and per-perspective cameras survive requests.
  displayRead ??= {
    ...read.graph,
    graph: { ...read.graph.graph, groups: [], relations: [] },
    targets: new Map(),
    relationDetails: new Map(),
  };

  const availablePerspectives: SuiteMatterGraphPerspective[] = historical
    ? ['matter', 'documents']
    : ['matter', 'documents', 'domain', 'panorama'];
  return (
    <SuiteMatterGraphView
      ...
      perspectiveError={Boolean(directoryPage?.error)}
      onRetryPerspective={directoryPage?.retry}
      nextDirectoryPage={
        directoryPage?.nextCursor ? directoryPage.loadMore : undefined
      }
      directoryLoading={directoryPage?.loading}
      perspective={perspective}
      onPerspectiveChange={setPerspective}
      availablePerspectives={availablePerspectives}
      revision={read.revision}
      timelineEvents={timeline.events}
      timelineSources={sources.sources}
      timelineLoading={sources.loading}
      onExpandTimelineSource={sources.expandSource}
      onOpenEventTimeline={openEventTimeline}
      onOpenWiki={openWiki}
