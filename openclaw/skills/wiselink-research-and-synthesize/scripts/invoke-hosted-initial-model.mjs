...
export async function invokeHostedInitialModel(
  ...
  const promptVersion =
    operation === 'EXTRACT_APPLICABILITY'
      ? WISELINK_APPLICABILITY_PROMPT_VERSION
      : 'wiselink-initial-generation@r09.c30';
  const systemMessage = {
    role: 'system',
  ...
  let messages = [
    systemMessage,
    { role: 'user', content: JSON.stringify(operation === 'TRANSLATE'
      ? translationGenerationInput(modelInput) : modelInput) },
  ];
...
/**
 * Preserve every original unit and the complete rule pack in native history.
 * The model returns indices/text; opaque binding IDs are restored and checked
 * against the original validated Host input, never generated by the model.
 */
export function translationGenerationInput(input) {
  return {
    sourceUnits: input.sourceUnits.map(({ kind, text }, index) => ({ index, kind, text })),
