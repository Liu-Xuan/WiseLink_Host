...
const OUTPUT_GUIDANCE = {
  ...
  EXTRACT_APPLICABILITY:
    'Return only wiselink.3_1.applicability_ast_candidate.v1 with expressions[{expressionId,sourceRefIds,extractionStatus:"extracted",expressionAst}]. Use this input astVocabulary exactly. If a bound source condition cannot be reliably interpreted, retain its exact expressionId/sourceRefIds and use extractionStatus="extraction_failed" or "not_supported" with expressionAst=null. Never invent a true/false AST or use no_rule_found to erase a bound condition. Do not output aircraft decisions, Fleet facts, target levels or content refs. Express only source-bound applicability conditions; unknown facts are not false. For applicability_task.v2, the verified English sourceExpressions and sourceContext are the primary input. A Chinese translation is optional assistance, never a prerequisite or authority. Preserve target bindings and extract conditions only; Host alone evaluates them against controlled aircraft and Fleet facts.',
  EVALUATE_JOBAID:
