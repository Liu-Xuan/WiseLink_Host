...
function matterReviewGuidance(input) {
  return [
    ...
    'matterWorkingDelta always includes updateKind, changeSummary, nextFocus, claimDelta, readingPresentation, openQuestionDelta, reviewConditionDelta, coverageUpdates. When the Host advertises problemWorkSchema it also accepts the problemWork field described below. nextFocus is null to retain the focus or {question,targetRefs}. claimDelta and readingPresentation are both null to retain the exact current result, or both objects to revise it. Host supplies the scope, identities, versions and CAS; never generate those control bindings.',
    ...(input.input?.context?.matterWorking?.problemWorkSchema ? [
      'This Host supports complete JobAid work in matterWorkingDelta.problemWork. For a substantive assessment update, supply that local issue-work update and set claimDelta=null and readingPresentation=null. Host derives stable Matter issue identities, the claim delta and the reading from the saved complete work; do not duplicate those outputs. Use context.matterWorking.previousProblemWork as the exact prior investigation, retaining unaffected issues, measures, dependencies and method limitations. Coverage-only updates omit problemWork. Member previousProblemAssessment is candidate context with its own subject; it is not this Matter’s prior work and must not be copied as target facts.',
