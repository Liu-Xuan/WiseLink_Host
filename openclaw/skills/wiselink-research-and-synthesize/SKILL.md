...
- Skill：`wiselink-research-and-synthesize@r09.c116`
...
- `workItemId` 用于 INITIAL_ANALYSIS 的 Host MCP 控制面；c116 的 JobAid 与 Overall 阶段还将同一
  WorkItem ID 传给来源读取、工作保存与读回、attempt 状态、心跳、取消及最终提交，
  由 Host 与持久化 attempt 的 tenant/WorkItem 联合核对。该 ID 不发送给模型。
  额外 WorkItem 作用域默认关闭，且当前仅支持分别受控的 JobAid 与 Overall 单阶段；
  翻译、适用性与 Review 不由这项配置获得额外任务权限。INTERACTIVE_REVIEW 入口只接收
...
- `skillVersion=wiselink-research-and-synthesize@r09.c116`
