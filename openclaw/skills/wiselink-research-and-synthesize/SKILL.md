...
- Skill：`wiselink-research-and-synthesize@r09.c117`
...
- `workItemId` 用于 INITIAL_ANALYSIS 的 Host MCP 控制面；JobAid、Overall 与 c117 的适用性阶段还将同一
  WorkItem ID 传给来源读取、工作保存与读回、attempt 状态、心跳、取消及最终提交，
  由 Host 与持久化 attempt 的 tenant/WorkItem 联合核对。该 ID 不发送给模型。
  额外 WorkItem 作用域默认关闭，当前支持分别受控的 JobAid、Overall 与适用性单阶段；
  适用性 begin 仅传精确 contextRef，Host 将其绑定到唯一 WorkItem，并要求该 WorkItem 有持久化的受控
  机号和评估日期。不得回退到旧 WorkItem 的全局目标，也不得把 WAITING_INPUT 当作适用性成功。
  翻译与 Review 不由这项配置获得额外任务权限。INTERACTIVE_REVIEW 入口只接收
...
- c43 将仅含数字行号或计数的校验后缀纳入上述纠正分支，例如 `DYNAMIC_RULES_RULE_RESULT_ROW_BUDGET_EXCEEDED:2`；任意文本、来源或敏感值后缀仍直接拒绝。模型返回前须按 Host `maxRowUtf8Bytes` 检查每行 JSON 的 UTF-8 字节数；纠正时检查全部行，只压缩冗余表述，保留实质事实、条件、发现和全部准则。适配器不截断行、不扩大预算。
...
当前 Review 驱动使用以下六个工具；Aily 只在 Host 声明本轮用户授权可用的 CHAT 中提供：
...
c65 在 JobAid 复核的模型传输中只要求 answer；没有新增条目时可省略 sourceRefs、missingInputs、candidateEvidenceRefs、warnings，驱动按协议表达为空集合。已提供的值不被替换，空字符串、null、错误类型和错误封套仍拒绝。没有退役问题可省略 retiredIssues；省略 unchangedIssueKeys 仍必须通过原有问题完整分区核对，不自动推断任何旧问题已保留。网关带已知工具失败横幅的 idle-timeout 文本准确报告超时，不重试、不保存。
...
也不等于原文已验证。授权缺失、检索失败或仍在运行必须说明。不得调用写入、发消息或审批能力。
...
CHAT 与 Matter 使用原生 `tool_choice=required`；JobAid c5 沿用 `auto`，但驱动同样要求实际收到唯一的读取或候选函数，纯文本不作结果。c37 的 Matter 输出函数只有 `candidateJson`
...
- `skillVersion=wiselink-research-and-synthesize@r09.c117`
...
已失败或超时的 Matter 可通过既有 `begin_matter_assessment` 的 `recoveryAttemptRef` 请求新 attempt。Host 重新核验 actor、Matter 修订、工作修订、精确旧工作与输入版本；只复用旧任务捕获的模型和 Host 持久化的实际来源读取回执。取消或仍有效运行的任务不能恢复，旧期限和历史不改写。

消费者只从 Host 指定的旧 attempt 检查点读取完整成功的 SAVE_WORK/FINISH 模型响应，核对 inputHash、模型、消息和响应参数绑定。旧候选通过新 attempt 的正常 SAVE_WORK 校验与 CAS 保存，再由官方模型会话续接；不会把本地候选当作已经保存的工程工作。缺失、未知或绑定不符即停止，不重新生成旧候选。Host 拒绝来源时只反馈候选中已经存在的精确 sourceRef，不能猜测引用或删除有依据的分析来绕过校验。
