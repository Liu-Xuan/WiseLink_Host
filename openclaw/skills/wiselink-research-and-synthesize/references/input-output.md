...
ResultEnvelope.skillVersion = wiselink-research-and-synthesize@r09.c113       # 仅普通 Hosted 分析的实际安装包版本
toolVersions.wiselink-openclaw-engineering-assessment = 1.2.0
promptVersion = 当前实际运行非空版本
```

`ENGINEERING_ISSUE_CORRECTION` 使用 Host 记录的 `producer.kind=OFFICIAL_PLUGIN` 及专用实例身份；其 `modelVersion`、`skillVersion` 和未报告的 `inputUnits`/`outputUnits` 可为 `null`，`promptVersion` 取专用实例版本。不要把 c103 写成该插件的 Skill 产物。
